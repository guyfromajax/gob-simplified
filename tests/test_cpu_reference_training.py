"""Locks the CPU-reference-training coupling: the tuned team-wide base
(_CPU_REFERENCE_BASE) and the frozen coaching-quality reference are two constants
that MUST agree, so CPU scores ~1.0 per position and the league sits on the ladder
once quality is live. A change to either the reference (weights/cap/breadth) or the
...
def _cpu_qualities():
    from BackEnd.api.franchise_routes import (
        _CPU_REFERENCE_BASE, _CPU_FOCUS_AMP_MEAN, _cpu_reference_top3,
    )
    out = {}
    for p in POS:
        top3 = set(_cpu_reference_top3(p))
        assert len(top3) == 3, f"{p}: reference top-3 must be exactly 3 (custom focus requires it)"
        eff = {a: _CPU_REFERENCE_BASE[a] * (_CPU_FOCUS_AMP_MEAN if a in top3 else 1.0)
...
def test_cpu_reference_training_scores_one_per_position():
    ...
    Measured fit: max|q-1| ≈ 0.016, spread ≈ 0.027 (tolerances include margin)."""
    qs = _cpu_qualities()
    for p, q in qs.items():
        assert abs(q - 1.0) < 0.03, (
            f"{p}: CPU coaching quality {q:.3f} drifted from 1.0 — the tuned base and the "
            f"frozen reference no longer agree; re-fit _CPU_REFERENCE_BASE.")
    spread = max(qs.values()) - min(qs.values())
    assert spread < 0.04, f"per-position CPU quality spread {spread:.3f} too wide"
