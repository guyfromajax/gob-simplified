...
test.describe('submit summary', () => {
  ...
  test('REVERSED: only the SAVE fires before the summary — running is a separate choice', async ({ page }) => {
    ...
    // now saves and asks; the FCC's "Run Recruiting Day" is what commits.
    ...
    // The other half — that the FCC's handoff (?action=run) DOES fire it and opens the
    // reveal — is covered in signing-reveal.spec.js. Pressing it here navigates, which
    // tears down __writes.
  });
