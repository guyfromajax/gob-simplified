...
**Document status:** Working implementation spec (engine work in progress). This file will evolve into the **standard reference** for court position checkpoints and snapshot payloads once the position ledger is implemented and adopted.
...
Fast break is resolved along **two disjoint entry paths** in code:
...
`resolve_fast_break_logic` **returns early** into `resolve_rim_runner_fast_break` when `rebound` and `fb_play_key in (RIM_RUNNER, THIRTY_TWO)` (see `phase_resolution.py` around the Rim Runner branch). So a given turn uses **one** of these functions end-to-end, not both.
...
|-----------|----------------|----------------------|
| Fast break `DEFENSIVE_STOP` (early return before shot/TO/foul branch) | Full turn with `animations`, no snapshot yet | `FAST_BREAK` + `post_stopper` or `fb_logic_defensive_stop` phase |
