...
### Important Distinction: `team_id` vs `user_team_object_id`

**For Developers:** Understanding this distinction is critical for consistent navigation and data access.

- **URLs/Navigation:** Always use `team_id` (ObjectId string) in URL parameters and navigation code
- **Database:** Stored as `user_team_object_id` in game mode documents (tournament/franchise)
- **Why the difference:** The codebase standardizes on `team_id` for all navigation and URL parameters for consistency, even though the database field is named `user_team_object_id`
- **Resolution:** When reading from database, map `user_team_object_id` → `team_id` for URL params. When writing to database, map `team_id` → `user_team_object_id` for storage.
