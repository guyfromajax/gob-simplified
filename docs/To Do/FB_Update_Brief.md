...
3. Triangle (will be implemented in the near future)
And we will continue to track stats for FBs after a steal as "After Steal".

**Fast Break Stat Tracking**
- **Implemented (backend):** `scouting_data["offense"]["fast_break_plays"]` with per-play **`A`/`S`** (`covert_release`, `rim_runner`, `triangle`, `after_steal`); increments align with **`Fast_Break_Entries`** / **`Fast_Break_Success`**. Turn field **`fast_break_play`**. Player offense: **`FB_A`/`FB_S` only** (`FB_F`/`FB_N` retired). **Box Score scouting lines** = follow-up UI.
...
- Field name **`triangle`** in code / data; label **"Triangle"** in UI
...
- **Box Score Scouting Notes (implemented):** Fast Breaks section after Defense Play Calls — **Total** + Covert Release / Rim Runner / Triangle / After Steal (see `box-score.js`). Prior structure was:
