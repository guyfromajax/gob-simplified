...
- **Root cause (init-game string _id):** Init-game creates the game doc with `_id` as **string** (`generate_game_id()` returns 24-char hex). We always `$set` with `{"_id": ObjectId(...)}`, so we never matched and updated 0 docs. **Fix:** `_set_team_attribute_changes_on_game(game_id_str, tac)` tries string _id first, then ObjectId; logs which matched. Use it in both complete-week paths. Also load game in `update_team_attributes_after_game` by string _id if ObjectId lookup fails.
