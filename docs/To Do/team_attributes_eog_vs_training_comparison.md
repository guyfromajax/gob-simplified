...
| `fight` | Win/loss only: win `-1 to +1`, loss `-3 to -1`. | Strength + Conditioning contributions use fight buckets: `0 -> -4 to -3`, `1 -> -3 to -1`, `2 -> -1 to +1`, `3 -> +1 to +2`, `4 -> +2 to +3`, `5+ -> +2 to +4`. Breaks can subtract at `4-5`. Any Culture Builder archetype also adds flat `0 to +1`. |
| `discipline` | Compare team `(F + TO)` to opponent `(F + TO) + 8`: lower `0 to +1`, equal `-1 to 0`, higher `-3 to -2`. | Inside/Outside defense + Passing + BH at **0.25×** per point, summed, half-up → same **0–5 bucket table as Fight** (`0 -> -4..-3` … `5+ -> +2..+4`). Breaks can subtract at `4-5`. Any Authoritarian archetype also adds flat `0 to +1`. |
