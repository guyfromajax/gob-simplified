...
session.assign_points(TrainingCategory.OFFENSIVE_DRILLS.value, {"inside": 1, "outside": 1})
...
session.assign_points(TrainingCategory.TEAM_BUILDING.value, 2)
session.assign_points(TrainingCategory.FILM_STUDY.value, 1)
session.assign_points(TrainingCategory.SCRIMMAGE.value, 3)
session.assign_points(TrainingCategory.BREAKS.value, 2)  # ← allows zeroing to be avoided
