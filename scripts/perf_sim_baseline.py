...
def run_cpu_game(fid, m, seed=None):
    ...
    away_score, home_score, summary = _run_franchise_cpu_full_simulation_core(
        fid, m["home_id"], m["away_id"], m["home_name"], m["away_name"], seed=seed
    )
...
def run_ps_game(job, seed=None):
    from BackEnd.practice_squad.sim import run_ps_full_simulation
    t0 = time.perf_counter()
    away_score, home_score, summary = run_ps_full_simulation(**job, seed=seed)
    return time.perf_counter() - t0, away_score, home_score, summary
...
def main():
    ...
    # Determinism needs a stable string-hash seed too: set iteration order varies
    # per process otherwise. PYTHONHASHSEED must be set before interpreter start,
    # so re-exec once when seeding was requested without it.
    if args.seed is not None and os.environ.get("PYTHONHASHSEED") != "0":
    ...
    def run_batch(kind, items, runner, label_of, workers):
        ...
        def _seed_for(idx):
            return None if args.seed is None else args.seed + idx

        def _record(item, idx, dur, away_score, home_score, summary):
            # Stat rows carry NO timing: they are the basketball regression anchor
            # and must be byte-identical across runs of the same seed. Wall time
            # lives in game_times / the phase JSON instead.
            meta = {"kind": kind, "matchup": label_of(item), "seed": _seed_for(idx)}
            game_times.append({**meta, "wall_s": round(dur, 4),
        ...
        if workers <= 1:
            for idx, item in enumerate(items):
                try:
                    dur, a, h, s = runner(item, _seed_for(idx))
                    _record(item, idx, dur, a, h, s)
                    print(f"  [{kind} {idx+1:>3}/{len(items)}] {dur:6.2f}s  "
                          f"{label_of(item)}  {a}-{h}", file=sys.stderr)
                except Exception as e:
                    failures.append({"matchup": label_of(item), "err": f"{type(e).__name__}: {e}"})
                    print(f"  [{kind} {idx+1:>3}] FAILED {type(e).__name__}: {e}", file=sys.stderr)
        else:
            from concurrent.futures import ThreadPoolExecutor, as_completed
            if args.seed is not None:
                print("  ⚠️  --seed with workers>1 is NOT reproducible: the global RNG "
                      "is shared across threads. Use --workers 1 for exact diffs.",
                      file=sys.stderr)
            with ThreadPoolExecutor(max_workers=workers) as ex:
                futs = {ex.submit(runner, item, _seed_for(idx)): (item, idx)
                        for idx, item in enumerate(items)}
                done = 0
    ...
    payload = {
        ...
        "mode": args.mode, "workers": args.workers, "seed": args.seed,
    ...
    rj.write_text(json.dumps({
        ...
        "git_dirty": payload["git_dirty"], "seed": args.seed,
        "franchise_id": args.franchise, "week": args.week,
        "rows": stat_rows,
