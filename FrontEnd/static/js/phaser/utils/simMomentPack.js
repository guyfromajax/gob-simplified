/**
 * Moment copy FALLBACK pack — DATA, not code.
 *
 * The source of truth is /sim-moment-copy.md, which the broadcast fetches at runtime so copy
 * edits ship without a code change. This pack is used only when that fetch fails (offline or a
 * downloaded build with no server). Keep it in sync by exporting the .md, and never write card
 * copy anywhere else — no card string belongs in rendering or engine source.
 */

export const MOMENT_PACK = {
  version: '2026.08.a',
  categories: {
