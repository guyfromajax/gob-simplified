...
from BackEnd.constants.momentum import (
    ...
    MO_FT_MIN_ATTEMPTS,
    MO_FT_SECOND_CHANCE_MULTIPLIER,
...
def resolve_free_throw_logic(game):
    ...
    # (2 × MO) × randint(*MO_FT_SECOND_CHANCE_ROLL), clamped to [0,100]; a 1–100 roll
    ...
    if not makes_shot:
        ...
        second_chance_threshold = (
            second_chance_pct
            + (MO_FT_SECOND_CHANCE_MULTIPLIER * ft_mo) * random.randint(_sc_lo, _sc_hi)
        )
...
def _hco_commitment(man_xy, def_pt, bh_xy, basket_xy):
    """S3a / Goal 2 (Dynamic_MM_Brief §6A) — decompose ONE defender's offset from the man he guards into
    the geometric commitment the offense reads. Returns ``{cushion, denial, sag}``:
      • ``cushion`` = ‖defender − man‖ — how far off his man he sits (this IS S1's openness, reused).
      • ``denial``  = (def − man) · unit(BH − man) — signed, + = ball-side (overplaying the passing lane
        → the backdoor lane behind him is open). The locked ball-axis-projection trigger for `backdoor`.
      • ``sag``     = (def − man) · unit(basket − man) — signed, + = toward the rim (help/drive lane).
    PURE geometry off the frozen grid — no attributes, no roll (execution skill enters only at the finish
    contest, per the geometry-only-first decision). A degenerate axis (man on the ball / on the rim) → 0."""
    import math
    vx = float(def_pt["x"]) - float(man_xy["x"])
    vy = float(def_pt["y"]) - float(man_xy["y"])

    def _proj(target):
        ax = float(target["x"]) - float(man_xy["x"])
        ay = float(target["y"]) - float(man_xy["y"])
        n = math.hypot(ax, ay)
        return (vx * ax + vy * ay) / n if n > 1e-6 else 0.0

    return {"cushion": math.hypot(vx, vy), "denial": _proj(bh_xy), "sag": _proj(basket_xy)}


def _hco_commitment_map(off_lineup, def_xy, coord_fn, bh_pos, bh_xy, basket_xy, off_to_def):
    """S3a — the commitment (§6A) for every MAN matchup on the frozen grid → ``{off_pos: {cushion,
    denial, sag}}``. Man-first (the zone commitment read is S4, alongside zone posture). The BH's own
    ``denial`` is forced 0 (he holds the ball → there is no passing lane to deny)."""
    out = {}
    for off_pos in off_lineup:
        if not off_lineup.get(off_pos):
            continue
        d_pt = def_xy.get(off_to_def.get(off_pos, off_pos))
        if not d_pt:
            continue
        c = _hco_commitment(coord_fn(off_pos), d_pt, bh_xy, basket_xy)
        if off_pos == bh_pos:
            c["denial"] = 0.0
        out[off_pos] = c
    return out
...
# S3b — cushion → should_shoot decision-openness. Below MIN the defender still contests (no lift);
# at/above OPEN the look is wide open (full lift). Ramps linearly between. LIFT_MAX matches the legacy
# froze bonus (20.0) it generalizes. Tunable (S3e).
STEP_IN_OPENNESS_MIN = 5.0
STEP_IN_OPENNESS_OPEN = 10.0
STEP_IN_QUALITY_LIFT_MAX = 20.0


def _hco_openness_map(commitment):
    """S3b (Goal 2) — turn each matchup's S1 cushion (from the stamped `_commitment`, §6A) into a
    PER-PLAYER `should_shoot` openness lift, so the shoot DECISION reads real space: an open shooter
    takes the conceded look (step-in) and an open off-ball man becomes the best dish (open-man read).
    (The make probability already reads real distance — S1 Part C — so this is the decision half.)
    Empty/None `_commitment` (flag-off, zone, unstamped) → empty map → `should_shoot` uses its legacy
    scalar openness → byte-identical."""
    if not commitment:
        return {}
    span = max(1e-6, STEP_IN_OPENNESS_OPEN - STEP_IN_OPENNESS_MIN)
    return {
        pos: STEP_IN_QUALITY_LIFT_MAX * max(0.0, min(1.0, (float(c.get("cushion", 0.0)) - STEP_IN_OPENNESS_MIN) / span))
        for pos, c in commitment.items()
    }


# S3c — backdoor read. Fires when an OFF-BALL man's defender overplays the passing lane by ≥ the bar
# (denial, ball-axis) AND the rim is unprotected (no help within the landing radius). Tunable (S3e).
BACKDOOR_TRIGGER_BAR = 4.0          # denial (grid) — how hard the defender must overplay before a backdoor is on
...
def _hco_backdoor_read(commitment, def_xy, bh_pos, basket_xy, defender_reads, off_to_def):
    ...
    if not commitment:
        return None
    # Shared rim clearance: any help sitting on the basket kills every backdoor this step.
    for d in def_xy.values():
...
# S3 — DYNAMIC good-read threshold for the two NON-INSIDE altered actions, by the defender's distance `d`
# to the man he guards (his frozen-grid cushion, which reflects posture: deny ~2 / loose ~4–5). Replaces
# the flat 110 for these actions. Backdoor: close defender → HIGHER threshold → more poor reads → cutter
# open (backdoors beat DENY). Jab: loose defender → higher threshold → more bites → pop open (jabs beat
# LOOSE). `COEF` is the per-grid swing (3 = subtle; raise for a stronger distance effect). Tunable.
...
def _hco_select_altered_action(player, location, off_eff, rng):
    """S3 altered actions — the per-non-BH-player OFFENSIVE trigger + selection (Dynamic_HCO_System.md §
    Altered Actions). Rolls `randint(1,100) < 0.8·IQ + 0.2·CH + off_eff`; below → he performs, else None
    (stationary). If performing, ONE action chosen at random by his location: **inside** → post up / flash,
    **outside** → backdoor / jab step. Returns the action name (`"backdoor"|"jab step"|"flash"|"post up"`)
    or None. (Only `backdoor` is dispatched today; jab step / flash / post up land in build-order #4.)"""
    attrs = getattr(player, "attributes", {}) or {}
    threshold = 0.8 * attrs.get("IQ", 0) + 0.2 * attrs.get("CH", 0) + float(off_eff or 0)
    if rng.randint(1, 100) >= threshold:
        return None  # stationary
    if _is_inside_location(location):
        return rng.choice(["post up", "flash"])
    return rng.choice(["backdoor", "jab step"])


_INSIDE_TARGET_SPOTS = ("basketSpot", "midLane", "lower lowPost", "upper lowPost", "lower midPost", "upper midPost")


def _hco_backdoor_target(is_away_offense, rng):
    """S3 backdoor — a RANDOM inside landing spot (location string + display coords)."""
    from BackEnd.engine.attack_drive_clearance import _spot_display_coords
    loc = rng.choice(_INSIDE_TARGET_SPOTS)
...
def _hco_altered_read_threshold(action, dist):
    ...
    harder read → cutter OPEN); **jab step** = `BASE + COEF·d` (loose → higher → harder → bites → pop OPEN).
    Replaces the flat 110 for these two. Falls back to 110 for anything else."""
    if action == "backdoor":
        return BACKDOOR_READ_BASE - ALTERED_READ_PROX_COEF * float(dist)
    if action == "jab step":
        return JAB_READ_BASE + ALTERED_READ_PROX_COEF * float(dist)
    return 110.0


def _hco_nearest_def_dist(point, def_xy, exclude=None):
    """S3 — distance from `point` to the NEAREST defender on the frozen grid (openness = min gap to ANY
    defender → folds help defenders in automatically, no clear-rim gate). `exclude` skips a def_pos."""
    import math
    best = float("inf")
    for dp, d in (def_xy or {}).items():
        if not d or dp == exclude:
            continue
        best = min(best, math.hypot(float(point["x"]) - float(d["x"]), float(point["y"]) - float(d["y"])))
    return best
...
def _hco_lift_from_cushion(cushion):
    ...
    span = max(1e-6, BACKDOOR_OPENNESS_OPEN - BACKDOOR_OPENNESS_MIN)
    return BACKDOOR_QUALITY_LIFT_MAX * max(0.0, min(1.0, (float(cushion) - BACKDOOR_OPENNESS_MIN) / span))
...
def _resolve_hco_offense_shot_dynamic(skeleton, game, off_lineup, def_lineup, is_setplay=False,
    ...
    from BackEnd.engine.motion_step_decision import (
        ...
        SHOOT, KICKOUT_SHOOT, HOT_READ_SHOOT, SUBTLE_MOVEMENT, FREELANCE_FORCED, BACKDOOR,
    ...
    # Turn-level gating (rolled ONCE per HCO turn). The OFFENSE roll is the STRATEGIC LEVER for altered
    # actions (S3): `offense_reads` = an "altering turn" at **alterations × 20%** (`randint(1,5) ≤ setting`
    # → 0→0% / 1→20% / 2→40% / 3→60% / 4→80%; **0 = never alter, run the set straight**). On an altering
    # turn the BH does SMs and altered actions ride them (see the SM branch). DEFENSE pressure is its own
    # independent aggression roll (kept `randint(0,4)`). The shoot decision (should_shoot) stays UNIVERSAL —
    # runs every step regardless; these rolls only gate the MOVEMENT matrix (subtle / disruption).
    ...
    for i in range(1, len(steps)):
        ...
        # Dynamic HCO Defense — S3a (Goal 2): stamp the geometric COMMITMENT of each man defender vs the
        # man he guards (cushion/denial/sag, §6A) on this reached step, off the SAME frozen grid the
        ...
        if not sm_precede:
            # S3b: feed the per-player cushion openness so the shoot/dish decision reads real space
            # (step-in + open-man dish). Empty off flag/zone → legacy scalar openness (byte-identical).
        ...
        # (S3 #2: the standalone pre-movement backdoor trigger was RETIRED here — the backdoor now fires as
        # one of the SELECTED altered actions on the BH's SM, in the SUBTLE_MOVEMENT branch below.)
        ...
        if action == SUBTLE_MOVEMENT:
            ...
            # S3b: teammates get their real cushion openness (open-man dish post-SM); the BH keeps the
            # SM-specific froze bonus (his post-SM position isn't on the stamped `_commitment`), never
            # below his cushion. Off flag/zone → froze-only map or empty → byte-identical.
            _open_map = _hco_openness_map(steps[i].get("_commitment"))
            if _alt_open:
                _open_map = {**_open_map, **_alt_open}   # S3: backdoor cutters' openness (0 if covered)
            if froze:
                _open_map = {**_open_map, bh_pos: max(20.0, _open_map.get(bh_pos, 0.0))}
            # S3: a fed backdoor cutter catches at his inside cut spot — route the dish there + tag the VO.
            _alt_locs = dict(locations)
            if _alt_moves:
...
def _resolve_full_court_press_dynamic_first_cut(game, def_scouting, text):
    ...
    return {
        ...
        "bat_oob_target": dict(dyn.get("bat_oob_target") or {}) if bat_oob else None,
