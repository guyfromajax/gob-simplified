...
# This file contains the ENTIRE shared_defense.py file (both MAN and ZONE defense)
# Download this file and preserve it before reverting to commit fc4573e3
# 
# Original location: BackEnd/utils/shared_defense.py
# Contains: 
#   - MAN defense functions (assign_bh_defender_coords, assign_non_bh_defender_coords)
#   - ZONE defense functions (ZONE_23_NORMAL, assign_all_zone_defenders, etc.)
# 
# NOTE: Zone defense code DEPENDS on MAN defense functions (assign_bh_defender_coords, etc.)
# So the entire file is preserved here to ensure all dependencies are included.
# 
# To restore: Copy this entire file back to BackEnd/utils/shared_defense.py

import random
from BackEnd.utils.shared import get_away_player_coords
from BackEnd.constants import HCO_STRING_SPOTS, HOME_RIM_COORDS, AWAY_RIM_COORDS

# ==================== MAN DEFENSE FUNCTIONS (Required by Zone Defense) ====================
...
def assign_non_bh_defender_coords(o_coords, ball_coords, aggression_level, is_away_offense, ball_spot="key", o_spot="key"):
    ...
    if ball_spot == "key":
        if o_spot in ["lower corner", "upper corner", "lower baseline", "upper baseline"]:
            ...
            x = ox + (random.randint(1,4) * x_direction)
...
def assign_zone_defender_coords(
    ...
    """
    # NOTE: This function requires assign_bh_defender_coords and assign_non_bh_defender_coords
    # from the original shared_defense.py file. They are NOT included here.
    # When restoring, ensure those functions are also present in shared_defense.py
