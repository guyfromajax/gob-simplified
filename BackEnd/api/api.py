...
try:
    ...
    def refresh_game_cache_from_db(gm: "GameManager", saved: dict):
        ...
        from BackEnd.utils.shot_split_tracker import restore_shot_split_from_saved, restore_fga_by_turn_type_from_saved
    ...
    def simulate_game(request: Request, body: SimulationRequest):
        ...
        # Shot diagnostics: shot-split chart + FGA-by-turn-type report.
        from BackEnd.utils.shot_split_tracker import format_shot_split_summary, format_turn_type_fga_summary
        print(format_shot_split_summary(game))
        print(format_turn_type_fga_summary(game))
    ...
    def _extract_saved_lineups(saved_doc: dict) -> tuple[dict, dict]:
        home_lineup = {}
        away_lineup = {}
        if not isinstance(saved_doc, dict):
            return home_lineup, away_lineup
        for player in saved_doc.get("players") or []:
            if not isinstance(player, dict):
                continue
            pos = player.get("pos")
            pid = player.get("playerId") or player.get("player_id")
            side = player.get("team")
            if pos in POSITION_LIST and pid:
                if side == "home":
                    home_lineup[pos] = str(pid)
                elif side == "away":
                    away_lineup[pos] = str(pid)
        return home_lineup, away_lineup

    def _body_get(body, key: str, default=None):
        if isinstance(body, dict):
            return body.get(key, default)
        return getattr(body, key, default)

    def _infer_resume_anchor_type(body, timeout_context: dict, anchor_type: str | None = None) -> str:
        if anchor_type:
            return anchor_type
        timeout_reason = str(timeout_context.get("timeout_reason") or "").upper()
        if timeout_reason == "FOUL_OUT" or timeout_context.get("foul_out_context"):
            return "foul_out"
        if _body_get(body, "resume_from_timeout", False) or timeout_context.get("timeout_next_play_type"):
            return "timeout"
    ...
    def _build_resume_anchor(
        ...
        return {
            ...
            "game_id": str(db_summary.get("game_id") or _body_get(body, "game_id", "") or ""),
            "quarter": int(db_summary.get("quarter") or _body_get(body, "quarter", 1) or 1),
            "clock": db_summary.get("clock"),
    ...
    def get_game_resume_state(
        ...
        if saved and isinstance(saved.get("resume_anchor"), dict) and not saved.get("is_final"):
            ...
            logging.warning(
                "🧭 [RESUME-ANCHOR-READ] game_id=%s type=%s quarter=%s clock=%s time_remaining=%s home_score=%s away_score=%s next_play=%s",
                normalized_game_id,
            ...
            return _resume_payload_from_saved_doc(
                ...
                saved=saved,
                has_cached_game=cached is not None,
                force_status="stoppage_anchor",
            )
    ...
    def simulate_quarter_endpoint(
        ...
        if game_id:
            ...
            if timeout_saved_state:
                ...
                logging.warning(
                    ...
                    game_id,
                    saved_quarter,
                    timeout_saved_state.get("clock"),
                    timeout_saved_state.get("time_remaining"),
                    timeout_next_play_type,
                )
        ...
        if (
            ...
            try:
                ...
                logging.warning(
                    "🧭 [RESUME-ANCHOR-SAVE] phase=pre_sim type=%s game_id=%s quarter=%s clock=%s time_remaining=%s home_score=%s away_score=%s next_play=%s resume_from_timeout=%s",
                    _anchor.get("anchor_type"),
                    game_id,
                    _anchor.get("quarter"),
                    _anchor.get("clock"),
        ...
        if is_final:
            ...
            print(format_turn_type_fga_summary(gm))
        ...
        logging.warning(
            ...
            game_id,
            frontend_summary.get("clock"),
            frontend_summary.get("time_remaining"),
            first_turn.get("clock") if isinstance(first_turn, dict) else None,
            first_turn.get("time_remaining") if isinstance(first_turn, dict) else None,
            first_turn.get("result_type") if isinstance(first_turn, dict) else None,
        )
    ...
    def simulate_turn_endpoint(request: Request, body: TurnSimulationRequest):
        ...
        try:
            ...
            # Court persistence: do not persist every mid-possession turn as the
            ...
            if game_id and quarter_complete:
                ...
                try:
                    ...
                    if not is_final:
                        ...
                        _anchor = db_summary["resume_anchor"]
                        logging.warning(
                            "🧭 [RESUME-ANCHOR-SAVE] phase=quarter_complete type=%s game_id=%s quarter=%s clock=%s time_remaining=%s home_score=%s away_score=%s",
                            _anchor.get("anchor_type"),
                            game_id,
                            _anchor.get("quarter"),
                            _anchor.get("clock"),
    ...
    async def call_timeout_endpoint(request: CallTimeoutRequest):
        ...
        try:
            ...
            return {
                ...
                "timeout_trace_id": timeout_response.get("timeout_trace_id"),
            }
        except Exception as e:
