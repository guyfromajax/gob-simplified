...
**Related:** See `../projects/Z-Completed/Unified_State_Persistence_Work_Plan.md` (archived) for complete implementation history and all phases (1.1-1.3, 2, 3, 4, 5.1-5.7)
...
- `../01_Game_Mode_Systems/Franchise_Mode_Overview.md` - Franchise lifecycle + state stores orientation
- `docs/Franchise_Mode/franchise_mode_architecture.md`, `docs/Persistence_Docs/COMMON_DATA_SET.md` - legacy docs tree; predate the 128-team / 36-week model, use with caution
...
**Season Progress:**
- `week`: Current week number (1-36: regular season 1-26, tournaments 27-34, recruiting/wrap-up 35-36)
- `current_week`: Alias for week
- `schedule`: Pre-generated schedule array `[[team_A_id, team_B_id], ...]` (26 regular-season weeks, 128 teams)
...
- `results`: Object with weekly summaries
  ```javascript
  {
    ...
    // ... up to week 26; tournament results live on the EOS structures
    // (conference_tournaments / region_tournaments / national_tournament)
  }
  ```
...
- `shot_threshold`: 90 to 100 in franchise init (center 110 for pill display)
...
**Initialization:** Team objects are created for all 128 teams when franchise is initialized via `FranchiseManager.initialize_season()` or lazily via `ensure_team_objects_exist()` when accessing Game Plan/Playbooks.
...
**Note:** Player attributes from training are loaded during game initialization: `load_roster(team_name, franchise_id=...)` (`BackEnd/utils/roster_loader.py`) reads FPD and merges franchise-specific attributes over the universal player base. See `../01_Game_Mode_Systems/Franchise_Mode_Overview.md` ("How a game consumes franchise state").
...
**For complete structure details, see:** `docs/Franchise_Mode/franchise_mode_architecture.md` Section 7 (legacy docs tree — predates the 128-team / 36-week model)

---

### Tournament Mode (Non-Gameplay) — **sunset mode**

> Tournament mode is sunset (see `../01_Game_Mode_Systems/Sunset_Modes.md`). This section is retained because the code remains in the repo until the tournament code purge, and the two-stage settings model below references the tournament master store.
...
- `shot_threshold`: 10 to 210 (randomized, center at 110 for pill display)
...
### Single Game Mode (Non-Gameplay) — **sunset mode**

> Single Game mode is sunset (see `../01_Game_Mode_Systems/Sunset_Modes.md`). Retained while the `mode="single"` code paths remain in the repo.
...
- `../projects/Z-Completed/Unified_State_Persistence_Work_Plan.md` - Complete implementation history (archived)
...
### Special Gameplay-Tracking Fields

Small, explicit fields written during gameplay for features that need facts the box score alone cannot reconstruct later. They live on `game_state`, are persisted by `summarize_game_state()`, and are restored from the saved game document on load. Add future special-tracking fields to this table.

| Field | What it is |
|-------|------------|
| **`opening_lineup`** | The five `player_id`s who started the game for each team (`game_state["opening_lineup"]`, shape `dict[str, list[str]]` keyed by team). Captured **once** at the Q1 opening tip and **immutable** for the rest of the game (the snapshot is skipped if already set, e.g. when restored from DB). Used for PGPC bench-vs-starter logic and any feature that needs "who started" instead of the end-of-game lineup. |

**`opening_lineup` implementation:** `snapshot_opening_lineups_to_game_state()` in `BackEnd/opening_lineup_snapshot.py` (writes `game_state`), called from `BackEnd/utils/opening_tip.py`; persisted by `summarize_game_state()` in `BackEnd/utils/shared.py`; restored from the saved doc in `BackEnd/api/api.py`. Consumed by PGPC (`BackEnd/pgpc_player_slot.py`, `BackEnd/pgpc_qualification.py`, `BackEnd/models/pgpc_snapshot.py`) — see `../06_GMO_Supporting_Systems/Press_Conference_System.md`.
