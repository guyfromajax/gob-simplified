...
- **Detect** — `save_result` already snapshots lead archetype before/after `finalize_game` (for the community-highlights row). On a change with a non-empty prior, it also sets user field `archetype_evolution_pending = <new key>` (`community_highlights.record_archetype_change_if_any`). First-time establishment (empty prior) does **not** set it.
