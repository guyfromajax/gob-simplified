# Soundtrack System (**verified 2026-06-13**)

> Verified vs `FrontEnd/static/js/musicController.js` — **100% accurate, no changes needed**. Confirmed: `FCC_TRACKS = ["scouting-track-1.mp3","scouting-track-2.mp3"]`, `GAMEPLAY_DEFAULT_TRACK="arcade-pulse-1.mp3"`, `GAMEPLAY_CRUNCH_TRACK="pixel-pulse-1.mp3"`, `CRUNCH_TIME_SECONDS=121`, `CRUNCH_SCORE_DIFF=6`, `TIMEOUT_LOOP_TRACK="Timeout_Loop.wav"`, `SCOUTING_AMBIENCE_KEY="gob_scouting_ambience_enabled"`, `DEFAULT_VOLUME=0.4`, `audio.preload="none"`; `evaluateGameplayTrack` logic (q>4 → Crunch; `sec < 121 && diff <= 6` → Crunch; else Default); and all referenced exports exist (`playFccTrack`, `resumeFranchiseTrack`, `clearFranchiseMusicState`, `startTimeoutLoop`, `clearTimeoutLoopState`, `resumeMusicForGamePlan`, `playGameplayTrack`, `evaluateGameplayTrack`, `pauseGameplayTrack`, `resumeGameplayTrack`, `isScoutingAmbienceEnabled`, `setScoutingAmbienceEnabled`, `tryStartScoutingAmbienceForCurrentPage`).
