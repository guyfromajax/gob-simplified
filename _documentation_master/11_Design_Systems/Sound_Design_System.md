...
**UI interaction sounds** used across the app — page-by-page catalog of which click/confirm/navigation sound fires on which button. Assets live under `FrontEnd/static/sounds/`. Playback uses root-relative `/sounds/` with `encodeURIComponent(filename)` for names with spaces/parentheses. Volume is typically 0.7; playback is wrapped in try/catch and `play().catch(() => {})` to avoid autoplay errors.

**Boundary:** gameplay/simulation audio — shot, pass, and reception cues, the shot-variant make/miss system, court announce stingers, whistles, airhorns, and the `gameSfx.js` runtime — is specced in [`../05_Features/SFX_System.md`](../05_Features/SFX_System.md). This doc covers only sounds fired by direct user interaction.

(`FrontEnd/static/sounds/_SFX_notes.md` is the original wiring work order with WORKING/NOT-WORKING status flags, retained for reference; this doc is the current catalog.)
...
> Scrimmage/Single Game and Tournament are **sunset modes**; their rows below describe wiring that remains in code until the mode purge.
...
> Simulation-driven sounds formerly listed here — the timeout airhorn and the foul / turnover / shot-clock whistles — moved to `05_Features/SFX_System.md` (June 2026).
