...
> **Status (verified 2026-06-13): forward-looking design spec — NOT yet built.** No court-generation renderer/pipeline exists in code yet. Runtime anchors verified against `courtClamp.js` (`CLAMP_BOUNDS` minX 5 / maxX 95 / minY 2 / maxY 49) and the 8 A1 reference court JPGs exist. This is now the **single** court-template spec (the former high-level `Court_Template_Spec.md` was folded in here and deleted, 2026-06-13).
...
**Feasibility conclusion:** all remaining team courts can be generated with 100% gameplay precision **only if** geometry is template-based, all line/basket positions stay fixed, only color + branding layers vary, and decorative variation is constrained to the small approved layout set below. AI image generation must **not** be used for the court base — the base is deterministic.

**Decoration layers — what varies:**

- **Always present:** team-colored lane / paint treatment; side-wall team-name typography; center-court team identity treatment.
- **Sometimes present:** small secondary logos inside the 3-point area (outside the lane); a wordmark / mascot wordmark between the center line and one 3-point arc; logomark-only layouts; center-logo-only layouts.

**Independently controllable layers:** outer hardwood tone, inner hardwood tone, paint/key color, and border/wall color are each separate variable layers (see Hardwood Variant System below).
