...
| `CAMP_WEEKS` | **3** | Franchise weeks 1..N are training camp (`is_camp_week`): 30-point cost budget, pre-training decay skipped, `CAMP_GAIN_SCALE` applied. | 2 → shorter camp; 4 → longer | LIVE (`training_shape.py`) |
...
| `CLASS_COST_MULT` | FR **1.0** / SO **1.1** / JR **1.25** / SR **1.4** | Multiplies attribute unit cost in `Σ units × cost × class_mult ≤ 24/30`. Seniors spend the same slider notches on fewer expensive attrs. | ↑ SR = tighter upperclass budgets | LIVE |
| Cost curve (`TRAINING_COST_*`) | derived cap **3.0**; explicit physical zeros **4.0**; universals ND/FT/IQ cost **1** | Budget cost per allocation unit from `TRAINING_COST_WEIGHTS` (not a gain damper). `cost = min(3, (w_max/w_a)^γ)` with γ=1; explicit 0-weight physical walls → 4.0. Full weight table in `BackEnd/constants/training_shape.py`. | re-prices which attrs a position can afford | LIVE |
...
| **`TRAINING_COST_WEIGHTS`** | per-position table in `training_shape.py` | Defines the cost landscape (and floor weight basis). Changing it re-prices every training plan and retunes which attrs get real floors. | cost curve + floor battery (`tests/test_training_shape_framework.py`) |
...
| `CLASS_COST_MULT` | FR 1.0 · SO 1.1 · JR 1.25 · SR 1.4 | Class multiplier on training cost spend. |
