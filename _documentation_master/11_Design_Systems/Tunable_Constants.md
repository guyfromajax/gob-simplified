...
> **INVARIANT (pinned by `tests/test_in_season_invariants.py`): a player trained at the REFERENCE allocation holds flat per attribute; NEGLECT (base-0) declines; FOCUS (base-4/5) gains.** Fix: the **reference band, points 1–3, all share the gain range (2,4)** (base-1/2 raised from (0,1)/(2,3)); base-0 stays (-2,-1), base-4/5 unchanged. A *global* decay/scale change was rejected — it over-inflates the mid band and reopens claw-back; the fix is local to the low band. Four assertions guard it: reference nets ~0 in-season, reference RT below the smallest rung increment (no claw-back), base-0 declines, and — because the offseason failed to restore the last one — the **FULL CYCLE (season + rollover) also holds**. Acceptance (perf_sim_baseline, same seed): trained-under-fix **FG% 36.97%** ≈ untrained 37.5% (was 33.1% pre-fix).

> **KNOWN PROPERTY (documented, not a bug):** points 1–3 now give identical gain, so the reference drifts every attribute mildly **down ~−0.7/season** in-season (scaled-gain rounding vs decay) — uniform, well inside the |Δ|<3 threshold, and **fully restored at rollover** (full-cycle ~0). The 1-vs-3 coverage distinction lives in the coaching-quality metric (offseason `f`), NOT in in-season gain; specialization comes from the FOCUS amplifier (intended), not from the base gradient. This is the **3rd in-season retune** (decay reduction → gain scale 0.26→0.18 → reference-band (2,4)); the model is now pinned by invariants so the next change to decay or gain breaks a test instead of eroding the league across seasons.

### Not exposed (fixed / not target-fittable)
`FAMILY_TIMING_WEIGHTS`, `FAMILY_TIMING_SHIFT`, `INTRA_FAMILY_GAMMA` (0.20), `NON_CORE_GROWTH_MULTIPLIER` (0.06), `PEAK_RUNG_WEIGHTS`, `PEAK_COUNT_DISTRIBUTION`, `RT_COMPRESSION_THRESHOLD`/`RT_SOFT_CAP` (95/130, near-inactive guard), `IN_SEASON_GAIN_SCALE` (0.18, fit so reference coaching lands ~+4-5 RT/season inside the band).
