...
**§7.2 "attributes move visibly" is about ATTRIBUTES, not RT.** RT is a weighted mean, so reference coaching that reallocates toward the high-weight core attrs moves the player page while barely moving RT: measured per-attribute |Δ| ≈ **0.52/week** at reference (net per-attribute ≈ 0), ≈ 0.62/week random. Visible movement is present under every policy.
