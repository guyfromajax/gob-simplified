...
> ⚠️ **HARD REQUIREMENT for pillar 3 — CPU must train the reference.** The whole normalization assumes CPU ≈ reference, so CPU scores 1.0 and the league holds on the ladder. That assumption is **currently FALSE**: CPU auto-train (`generate_random_training_allocations` + `generate_random_coaching_focus`, per team per week) allocates **randomly**, which scores well below 1.0 once coaching quality is live — measured (2026-07-30, postrecal-validation pool): random/CPU allocation nets ≈ **−0.8 to −1.0 RT/8wk** vs the reference/normal-coaching **+**. If quality goes live while CPU trains randomly, the **entire league drifts under the ladder**. Pillar 3 MUST make CPU auto-train allocate the reference (or archetype-modulated variants distributed *around* it, averaging to it), not a uniform random spread. This is a blocker for turning coaching quality on, not a nicety.
