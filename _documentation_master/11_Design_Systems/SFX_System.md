...
- **Status:** ✅ live (2026-07-14). Not gated by `GOB_DYNAMIC_HCO_DEFENSE`.
- **Trigger:** the ball-handler starts a drive to the rim — covers both drive-to-shoot and drive-and-dish.
- **Frequency:** fires on **~50% of drives** (`HCO_DRIVE_SFX_FIRE_PROB = 0.5`); the rest drive silently. A **seeded coin** (crc32 of `driver_id | step_index | timestamp | "drive_sfx_fire"`) with a *separate salt* from the variant pick → the fire/skip decision is uncorrelated with which clip plays, and stays SS&S-reproducible.
- **Files:** one of `sammy-drive.wav` / `braddock-drive.mp3` (`HCO_DRIVE_SFX_FILES`, vol `HCO_DRIVE_SFX_VOLUME = 0.8`). When it fires, the variant is picked by a **local seeded RNG** (crc32 of `driver_id | step_index | timestamp`) — SS&S-reproducible and it **never draws from the global stream**, so a cosmetic cue can never perturb shot outcomes.
...
- **Phase `playbackComplete`:** after turn animation finishes — `AnimationRouter` calls `signalQuarterEnded` for **every** turn (universal fallback after boundary tween drain). EOQ-specific handlers (Final Turn / FLSS hold, schema FT, Run Out) may fire earlier; per-turn dedupe prevents double-play. Plays on `quarter_ends_after` even when `clock_start === 0`.
