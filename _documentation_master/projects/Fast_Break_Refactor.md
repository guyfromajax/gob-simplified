...
| **2c hotfix — Diagnose & fix path 2 hang** | Required before re-attempting 2e step 1. Add temporary `console.log` markers inside `animateDefensiveStop` path 2 (entry, after-loop, before-await, after-await) to find which line hangs. Suspected causes: `tweenPlayerTo` returning a never-settling promise (e.g., `scene.tweens.add()` returns null on invalid target, NaN duration, or destroyed sprite). Once diagnosed, targeted fix; remove instrumentation. | Medium |
