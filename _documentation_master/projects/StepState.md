...
## ▶ RESUME HERE (checkpoint 2026-07-12)

> **Numbering:** formal "Staged plan" scheme below (Stage 0–3). Stage 2 (defender-grid sharing) shipped + verified. Since then: moment-teleport pin, walk-time seam (Option B), and the motion+setplay **resolver unification** shipped. Stage 1's **moment fusion** is SCOPED but DEFERRED (high-risk spine surgery — see below). Stage 3 open.

### Since the 07-11b checkpoint (all committed to develop, ahead of origin — PUSH)
- **Moment-teleport pin (`eb0408de6`)** — `_resolve_hco_moment_walk` fires foul/steal/turnover at a known step `i` but discarded it → `apply_stopper` used a random blast-radius step (ball snap-back). Now stashes `_hco_moment_stop_index=i`; `apply_stopper` consumes it (foul + turnover/steal branches). Fixes the DB-turnover-after-pass teleport. HCO-only pin; HCT/FCP untouched.
- **Walk-time interception seam / Option B (`b83f1ec51`)** — the walk-time contest ran before the grid was stamped (stamp lived only in coverage), so those picks used reconstruction. Extracted `_stamp_contest_defender_grid`; now called BEFORE the offense walk too. All HCO interception contesting (skeleton passes, dishes, hot-reads, coverage) reads the render grid, man+zone. No residual (dishes contest against the stamped `steps[i]`).
- **Resolver unification (`7b578cb36`)** — the motion + setplay dynamic resolvers were ~255-line near-duplicates; authoritative diff found EXACTLY ONE behavioral difference (the set-play `_setplay_recovery_roll`). Merged into `_resolve_hco_offense_shot_dynamic(..., is_setplay=False)`; `is_setplay` gates that fork, `_kind` the log labels. Deleted the 257-line duplicate; kept `_resolve_motion_/_resolve_setplay_offense_shot_dynamic` as thin delegates (stable test/caller API). Behavior-preserving (HCO subset failing set byte-identical to baseline). **This is the tractable core of formal Stage 1 — the fragmentation debt paid down; the moment now lands in ONE resolver, not two.**

### ⏸ Moment fusion — SCOPED, DEFERRED (2026-07-12). Why:
Goal: fold the on-ball moment into the unified per-step walk per Decision #1 (moment every step, moment-first, first-terminal-in-step-order wins) → the moment stops pre-empting a shot on a step the offense never reached. **Blocker:** `resolve_half_court_offense_logic` (~1300 lines) handles the moment's FOUR outcomes (STEAL / DEAD_BALL_TURNOVER / O_FOUL / D_FOUL) through **~450 lines of interleaved non-shot machinery** (`apply_stopper` 6906 → defender-override 6924 → event_type 7285 → ball-handler/defender/foul-player 7311 → foul resolution 7360 → downstream), which ALSO serves shot-clock violations + general fouls. The interception finalizer is a clean 73-line standalone ONLY because interception → just STEAL/BAT_OOB. A moment finalizer would have to re-implement that broad spine for 4 types — **not a clean extract; high-risk re-implementation.** Two heavy paths: **(A) reorder** so the resolver runs before the non-shot machinery; **(B)** big multi-outcome moment finalizer. Both = spine surgery. **Deferred:** the visible moment bug (teleport) is already fixed; the fusion's remaining benefit (terminal-step respect) is subtle. **Do it AFTER Stage 3** — Stage 3 removes the legacy + recalibration bypasses that make the spine branchy, so the fusion lands on a simpler spine with less risk. Path A preferred when we do it.

> **Historical note:** earlier this block called Stage-2 work "Stage 1 man/zone" — a local mislabel; it is **Stage 2**. Formal **Stage 1 (walk unification)** = the moment fusion above (partly done: resolvers unified; moment-into-loop deferred). **Stage 3 (bypass removal)** open.
