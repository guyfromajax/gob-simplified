...
### ⏸ Moment fusion — the DEFERRED CAPSTONE (formal Stage 1). Status after Stage 3:
Goal: fold the on-ball moment into the unified per-step walk per Decision #1 (moment EVERY step, moment-first, first-terminal-in-step-order wins) → the moment stops pre-empting a shot on a step the offense never reached. **Blocker (still true):** `resolve_half_court_offense_logic` (~1300 lines) handles the moment's FOUR outcomes (STEAL/DBTO/O_FOUL/D_FOUL) through **~450 lines of interleaved non-shot machinery** (apply_stopper ~6900 → defender-override → event_type → ball-handler/defender/foul-player → foul resolution → downstream) that ALSO serves shot-clock violations + general fouls. Two heavy paths: **(A) reorder** so the resolver runs before the non-shot machinery; **(B)** a big multi-outcome moment finalizer. **What Stage 3 changed:** the legacy shot path is gone → there is now ONE shot path (the dynamic resolver via the wrapper) to move, so **path A (reorder) is more tractable than when this was deferred.** NEXT STEP = a read-only re-scope of path A on the simplified spine before committing.
