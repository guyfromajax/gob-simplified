...
3. **← NEXT: HCT/FCP trap/press** (`Trap_Press_Positioning_Decision.md`) — the last position-authority gap. Engine snaps all 5 defenders to the full trap; render interrupts them → steal/foul fires from a defender not visibly on the ball (a *teleport*, not a `player.coords` read — the guard can't see it). Fix (§6 #4, design settled): size the collapse/converge beat by the **slowest mover** so the trap is reachable, matched engine/emitter rates → `end.coords` == logic == render. **GATE: user approved (2026-07-07/08) the *real-speed (slightly slower) trap close*, conditioned on a documented revert.** Ship behind a **per-family kill switch** (`GOB_HCT_REACHABLE_TRAP` / `GOB_FCP_REACHABLE_PRESS`, **default off / opt-in**) so the HCT feel can be reverted independently of FCP, gating POSITION/TIMING only (never the trap decision → revert is cosmetic, not a gameplay swing). Full rollback plan in `Trap_Press_Positioning_Decision.md`. Then prototype-verify (not fully sim-checkable). Note: the HCT moment engine is shared by the now-default Dynamic HCO system, so this work is adjacent.
