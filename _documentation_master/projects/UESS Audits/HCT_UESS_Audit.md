...
| 4 | MED | §8.4 | **Entry ball seeded by attachment (BH), not `final_ball_coords`.** `build_walk_up_step` hardcodes ball to `bh_id`; HCT never threads the prior rendered ball rest. Teleports on in-flight/loose prior ends, or `prior_final_bh_id==None` (→ *this* turn's BH). **SIP-Task 1 parity.** **AMENDED 2026-09-15:** that `None` → this-turn-BH path is the invent-holder. Policy 26 abstains on a missing stamp. The common path is closed; what remains is the **3 parked hops** (bugs.md item 51 — TIMEOUT emits no `final_ball_handler_id`, `:1118` takes `or bh_id`). The honest fix (seed walk-up from the loop's step-0 PG) retimes the step; no harness prices the body/T/clock change. Do not author an entry pass. | dynamic_hct_step_emitter.py:764-778; transition_bridge.py:350 |
...
> ⚠️ **Cannot sim-verify HCT in the mock** (needs trap-defense playbook config). Group B is novel clock logic (not parity-based like Group A), so Tasks 4 & 5 are **scoped for prototype-verified follow-up**, not blind-shipped. Only the strictly-defensive Task 6 shipped.

- ✅ **HCT-Task 6 (LOW #9) — DONE (2026-07-05).** Floored `shot_clock_remaining` at `max(0.0, …)` in all 4 emitter step-builders (dynamic_hct_step_emitter.py:324/441/524/616) → no negative shot clock rendered when a segment crosses zero. Purely defensive; regression-clean.
- ⬜ **HCT-Task 4 (HIGH #1) — SCOPED (needs prototype verification).** The engine (`dynamic_hct.py`) runs *before* the emitter and can't know the emitter-computed walk-up duration. **Fix:** in `compute_dynamic_hct_turn`, before the loop, estimate the walk-up travel time (BH `prior_final_coords[bh]` → `bh_target ~(44,y)` distance ÷ standard grid-rate — same basis `build_walk_up_step` uses) and pre-decrement `shot_clock` (and shift the 10-second `shot_clock_start` baseline) by it, so the `shot_clock <= 0` (dynamic_hct.py:2944) and 10-sec (:2952) terminals evaluate the *committed* clock. HCT-only (`skip_walk_up` for FCP). **Risk:** shifts *when* violations fire — verify in prototype that violations land at the right moment and don't over-fire.
