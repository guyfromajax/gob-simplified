...
**Group B — ball entry-seam (REFRAMED 2026-07-05 after measurement):**
- ⚠️ **The "seed from `final_ball_coords` ×4" recommendation was WRONG for FB — rejected.** FB deliberately seeds step-0 from the live **owner** coord, which is *correct*: DREB→FB is continuous by construction, and steal→FB is correctly at the **stealer**. `final_ball_coords` for a STEAL is the **victim's** coord (the rendered rest while the interception animation is still broken), so seeding from it would teleport the ball to the victim. Confirmed empirically: **0 detector fires across 124 DREB-path FB turns** — the entry seed is clean.
- ✅ **FB-Task 2 (MED-HIGH #3) — DONE.** Added a `[UESS SEAM]` entry detector at the FB callsite (turn_manager.py), scoped to **skip the documented STEAL case** (that entry teleport is the interception bug — see below) and guard the DREB/other entries against regressions. Verified quiet on the DREB path.
- 🟡 **#6 STEAL `final_ball_coords` — deferred to the steal thread (NOT an independent bug).** `build_final_ball_coords` faithfully returns the *rendered* rest (the victim) because the interception animation doesn't fly the ball to the stealer (the FCPHCT interception bug). Patching it to return the stealer would make it lie about the render. It resolves for free once the steal thread fixes the interception animation (then the last step shows the ball at the stealer). Do NOT patch in isolation.

**Group C — contest-frame divergence (HIGH #2 + MED-HIGH #4 + MED #7) — SCOPED, DEFERRED to a prototype-verified pass (2026-07-05):**
> ⚠️ **NOT sim-verifiable + touches make/miss.** Measured: the mock's FB shots (108/108) do NOT hit the universal drive-resolution path (`resolve_fb_drive_step`) — they use the render-synced legacy/geometry path — so the divergence can't be reproduced or measured in the mock. Group C touches the shot **contest** (→ make/miss), so a wrong fix would silently shift FB FG% with no test to catch it. Same risk profile as the deferred HCT clock tasks. **Do with prototype access, ideally folded into the reachability capstone** (it's the same "logic reads render" work as #8 + over-and-back).

- **FB-Task 4 (design, when picked up):**
  - **#2 (HIGH):** in `resolve_fb_drive_step` (fb_drive_resolution.py:311/462), compute the contest from the **rendered** `author_transition_end_coords` frame — the same planner the emitter uses (`after_steal_transition_positioning.py` via fb_drive_step_emitter.py:269) — instead of `_reachable_defender_ends(...)` toward `basketSpot`. This is the circular-dep pattern (contest decides before the emitter renders): compute the author frame in the resolver, use it for `_contest_at_shot` + rebound, then the emitter re-uses it → contest == render by construction. Only for the `author_offball_spread=True` paths (RR/CR/Triangle); After-Steal (`=False`) is already aligned.
  - **#4 (MED-HIGH):** Triangle spot-up contest (`rim_runner_fast_break.py:278-288`, `rim_runner_drive_integration.py:77-93`) reads live `player.coords` (end-of-DREB) — make it read the rendered setup `to` spots (`bh_def_to`/`rr_def_to`/`helper.to`, triangle_step_emitter.py:634). The clearest coord-consumer defect; safest sub-part if a partial is ever wanted.
  - **#7 (MED):** RR/CR rebounder selection reads the sampled-spot frame (`_build_offense_end_coords`) — route through `author_transition_end_coords`. Attribution-level (accepted-gap class, like Coord_Consumer #3/#4).
  - **Verify in prototype:** measure FB contested-rate + FG% before/after on the drive-step path (which the prototype exercises).
