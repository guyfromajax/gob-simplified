...
- [x] Confirm the *real-speed* trap/press close (slightly slower than the current instant-snap) is acceptable — **user approved, then made STANDARD 2026-07-08** ("it's what we need to do anyway"): the `GOB_HCT_REACHABLE_TRAP` flag was removed and the reachable terminal stopper is now unconditional (`dynamic_hct.py` `_emit_stopper`). Revert path is now a `git revert`, not an env toggle. Still prototype-verify the feel.
- [x] **HCT-Task 7b (terminal stopper) DONE + standard** (commit b662aa2a5 behind flag, then flag removed 2026-07-08). **HCT-Task 7a (converge beat-sizing = more full traps + slower — a real BALANCE change) NOT done, deferred.**
- [x] **FCP-Task 3 (press) — SNAP/TELEPORT GOAL DONE, no code change needed (traced 2026-07-08).** FCP delegates to the shared `dynamic_hct` loop (`turn_mode="fcp"`), so: the terminal stopper shares `_emit_stopper` → **already reachable via HCT-Task 7b**; the FCP-only engagement beat (`_apply_fcp_engagement`) is **already interrupt-based** (BH+PG, max-sized), not a snap. **No remaining snap/teleport in FCP.** The only remaining piece is the **shared** `hct_converge` beat-sizing = the deferred **Task 7a** balance change (higher-impact for FCP because `skip_walk_up` makes the converge cross the whole court → PG-sizing leaves far defenders short, so the press under-forms). Because the converge is SHARED, Task 7a would change both HCT and FCP unless gated by `turn_mode`. User chose (2026-07-08) to mark FCP-Task 3 done and treat the converge as the separate deferred Task 7a balance decision. `GOB_FCP_REACHABLE_PRESS` never needed (no snap to gate).

## Rollback plan (revert the trap-close feel if disliked)
**HCT update (2026-07-08): the `GOB_HCT_REACHABLE_TRAP` flag was REMOVED and the reachable terminal stopper made the standard, unconditional behavior** (user: "it's what we need to do anyway"). So for HCT the revert is now a **`git revert`** of the impl commits, not an env toggle. The plan below still applies to **FCP-Task 3** (build the press fix behind `GOB_FCP_REACHABLE_PRESS` first, evaluate, then optionally make standard the same way). Original design requirements (kept for FCP + as the record of how HCT shipped):
...
- **Gate POSITION/TIMING only, never the engine decision.** The flag wraps exactly the beat-sizing + the emitter's PF/C-sprint render (the coord/timing). The trap decision itself (who traps, the steal/foul roll) stays OUTSIDE the flag — the steal/foul is credited BEFORE the terminal beat. **Correction (measured 2026-07-08, HCT-Task 7b terminal stopper):** flag-OFF is byte-identical (path skipped), but flag-ON is **NOT** byte-identical — the honest longer trap-close consumes real GAME CLOCK, which butterfly-reshuffles subsequent possessions. It is **aggregate-neutral, not zero:** over 12 quarters ×3 seeds, steals 7→7 (0%), turns +0.1%, points −3.2% (one-game noise), i.e. a neutral unbiased reshuffle — the same character as the FB "one-time RNG reorder," NOT the "purely cosmetic" originally claimed here. So the revert (flag off) is exact; enabling it is a minimal, unbiased balance perturbation, not a gameplay swing but not a no-op either. Verify the neutrality over a larger sample when enabling in staging.
