...
- [x] **HCT-Task 7b (terminal stopper) DONE + standard** (commit b662aa2a5 behind flag, then flag removed 2026-07-08). **HCT-Task 7a (converge beat-sizing = more full traps + slower — a real BALANCE change) NOT done, deferred.** **FCP-Task 3 (press) still open** — build behind its own `GOB_FCP_REACHABLE_PRESS` flag (per the rollback plan below), separate from HCT.
