...
## Work plan
- **TO-Task 1 (HIGH §1) — SOFT MIRROR SHIPPED 2026-07-08, hard-reject is the follow-up.** The eligibility rule now lives backend-side: `GameManager._user_timeout_eligible(calling_team)` mirrors the FE `checkTimeoutEligibility` exactly (inbound → eligible; the MISS/BLOCK+DREB turn itself → not; HCO right after the calling team's OWN DREB with them on offense → eligible), evaluated against the backend's own `self.turns[-1]`/`[-2]`. `call_timeout` calls it for USER timeouts and **LOGS `🟠 [TIMEOUT ELIGIBILITY]` when the backend rule says ineligible but still ALLOWS it** — a soft mirror, chosen because the FE/backend can briefly disagree on the current turn (see the timeout-skew guard in `api.py`), so a naive hard reject risked false-rejecting a valid timeout. Verified: 10/10 unit cases match the FE rule; behavior unchanged (log-only). **Follow-up:** once the `🟠` logs confirm the FE and backend always agree in real play (zero false-positives), flip the soft log to a hard reject (`return None`) — the rule is then backend-authoritative and the FE check becomes pure UX. Files: `game_manager.py`.
