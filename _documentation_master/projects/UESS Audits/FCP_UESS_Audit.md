...
| 6 | LOW→consequential | §1/§8.3 | **`_advance` BH drive rate mismatch → over-and-back / violation reads.** Engine jumps the BH the full distance at **AG-drive rate**; emitter renders interrupted at **standard**. For a fast handler the engine BH ends **ahead** of the rendered BH, and the FCP-load-bearing reads run off the engine's forward pos: `frontcourt_established`, 10-second violation, **`is_over_and_back_pass`**. → engine thinks BH is in the frontcourt while the FE renders him crossing back → **over-and-back violation doesn't fire.** Shared `_advance` (hits HCT + FCP). **Likely the over-and-back bug reported in the other thread.** — **✅ FIXED in code (2026-07-06, code-verified):** the emitter renders the BH advance at the AG-drive rate (`dynamic_hct_step_emitter.py:279-287`) matching engine `_advance` (`dynamic_hct.py:2218`, same `_interrupted_coord`) → the violation reads the rendered coord. Not yet prototype-checked. | dynamic_hct.py:2511-2521, :3185 |
...
- **#6 over-and-back: ✅ FIXED in code (2026-07-06, code-verified)** — the emitter renders the BH advance at the AG-drive rate (`dynamic_hct_step_emitter.py:279-287`) matching engine `_advance`; `is_over_and_back_pass` / `frontcourt_established` read the rendered coord. Not yet prototype-checked (possible seam-drift residual — see [Trap_Press_Positioning_Decision.md](Trap_Press_Positioning_Decision.md)).
