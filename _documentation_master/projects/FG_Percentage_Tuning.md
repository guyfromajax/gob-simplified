...
- Central: `made = shot_score >= shot_threshold` (`BackEnd/models/shot_manager.py`)
- **Uncontested inside/attack** (within 11 grid of basket, not three): universal roll helper in `BackEnd/utils/uncontested_shot.py` — `make_threshold = 99 + offense discipline − defense fight`; make if `random.randint(1,100) < make_threshold`. Used by rim-box shortcut, motion attack uncontested, generic undefended inside/attack in `resolve_shot`, Steal-FB, HCT-FB, and OREB putback.
...
|---|---|---|
| HCO, Fast Break, Final Shot | shared `resolve_shot` (radius 11) | shared `resolve_shot` (uncontested inside/attack helper + undefended-outside bar) |
| HCT, FCP | own: radius 11 + x-trail ≤ 3 | own `dynamic_hct_shot.py` — uncontested helper + no undefended-outside bar |
| OREB | nearest defender always | inline in `shared.py` — uncontested helper when IQ read fails |
...
|---|---|---|
| **Base Final Turn** | normal end-of-clock shot completes in time | Standard `resolve_shot` — same as HCO (undefended-outside bar, uncontested inside/attack helper, else `score >= threshold`). `final_turn=True` but **no** heave/time penalty. (`phase_resolution.py:6296`) |
