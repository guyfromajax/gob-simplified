...
`conference_place`, `conference_size`. Derived from `calculate_franchise_standings` — the same
helper the Standings tab uses, so the two cannot disagree. Place orders by wins, then point
differential. `null` outside a franchise.
