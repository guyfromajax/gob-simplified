...
|---|---|---|
| **`equiv_v3_reference_f2a060488_manhelpshade.json`** | `f2a060488` (2026-09-22) | **CURRENT, both arms, both footings.** Man weak-side help shade ON by default: a man off-ball **help** defender (posture normal/loose) now sags toward the rim he is defending, scaled by how weak-side he is — the one rim-ward term becomes `HELP_BASKET_SHADE × (1 + weakness)`, so the strong side and a central ball are unchanged (0.20) and the far weak side doubles (0.40). `weakness` is `help_side_weakness`, **extracted** from `_apply_zone_help_shade` so zone and man share one ramp — no new constant, **no clamp**, nothing retuned, and posture does **not** scale the shade. Deny, the inside-man lock and the on-ball cushion are untouched. Weak-side defender→rim 13.25 → 10.71, strong side 15.24 → 14.94. **Both footings move, and every seed differs in all four cells** — unlike the zone help shade, man defense exists at `SEED_DEFENSES=0` too (an empty catalogue makes every zone call play man), so SD=0 carries 2.05× as many shaded placements as SD=1 and moves more, not less. See `reports/man-help-shade-flip-2026-09-22.md`. |
