# Handoff: Tutorial Experience — "Mastering GOB" Hub
...
Each topic will eventually get its own sub-page (a later build pass). This handoff
is **the hub only**.
...
- Each **Core Loop step** (Know Your Players, Set Your Game Plan, Train Your Team,
  Scout & Adjust)
- The hero **action button** (dynamic "next lesson")
- The **"Next up"** pill in Coach Sammy's card
...
2. **Hero** (2-col grid `1.55fr / 1fr`, gap 30px, vertically centered):
   - **Left:** eyebrow "GOB COACHING ACADEMY" (orange tick + uppercase tracked
     label); `<h1>` "MASTERING" / "GOB" stacked, Bebas Neue
     `clamp(54px,7.2vw,96px)`, `line-height: 1`, "GOB" in orange `#f79420`;
     CTA row: action button (dynamic next lesson) + ghost "Browse all topics".
   - **Right:** Coach Sammy card (`.panel`): round avatar (`/images/sammy_tutorial.png`,
     66px, 2px orange ring), "YOUR GUIDE" (orange, uppercase) / "Coach Sammy"
     (Bebas 24px) / quote (Inter 13.5px muted):
     *"We'll cue the next tutorial in line for you here. But you can browse any of
     the topics at any time — so you can learn GOB on your terms."* — followed by
     the **"Next up" pill** (`#sammy-next`, dynamic — see Interactions).
...
4. **Core Loop** section — eyebrow "START HERE", h2 "MASTER THE FUNDAMENTALS",
   right hint "Four pillars every coach should understand." Then a 4-up grid of
   numbered step cards (`.loop-step`): step 1 ("KNOW YOUR PLAYERS") has an **orange
   number chip** + "START HERE" tag; steps 2–4 are neutral. `→` connectors between.
   Steps map to category anchors but are **dead** (no real navigation).
5. **Topics** section — eyebrow "ALL TOPICS", h2 "MASTER THE COMPLETE COACHING
   SYSTEM", right hint "Browse the whole system. Each topic is a self-contained
   lesson — dip in anytime." Then a 2-col grid of four **category panels**
   (`.cat.panel`), each: header (icon chip + Bebas name + count) and a list of
   **topic tiles**. Tiles are dead this pass.
6. **Smart Reminders** band (`.panel`, 3px orange left accent): eyebrow "SMART
...
#### Component specs
**Topic tile** (`.tile`): flex row — 40px rounded icon chip · name (Inter 700, 15px)
+ badge · sub-copy (12.5px muted) · trailing `→` (or green ✓ when explored).
- **Priority** variant (`player-attributes`): orange-tinted bg
  `rgba(247,148,32,0.06)`, border `rgba(247,148,32,0.28)`, orange icon chip, and a
  `★ Master first` orange tag instead of a depth badge.
...
- **Dynamic "Next up"**: both the hero action button (`#cta-next`) and Sammy's pill
  (`#sammy-next`) show the first **un-explored** lesson in recommended order:
  `['player-attributes','game-plans','training','team-attributes','playbooks','scouting','recruiting']`.
  As lessons get marked explored, the cue advances; when all are explored it reads
  "All caught up" / "Every lesson explored". (Dead navigation this pass — but labels
  are live.)
