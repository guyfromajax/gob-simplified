...
/* [label, away, home, kind, betterIsLow, barPointsLow]
   betterIsLow drives which VALUE is emphasised white; barPointsLow drives which way the BAR pulls.
   They are separate on purpose: turnovers emphasise the team doing better but the bar marks the
   team with the problem. */
const TEAMSTATS=[
 ['FG%','43.1','47.6','tug',false,false],
 ['3PT',5,8,'tug',false,false],
 ['PTS IN PAINT',18,26,'tug',false,false],
 ['FAST BREAK',6,17,'tug',false,false],
 ['REBOUNDS',31,27,'tug',false,false],
 ['TURNOVERS',9,14,'tug',true,false],
 ['TEAM FOULS',7,11,'tug',true,true]];
...
function teamPanel(rows){
  ...
  const out=src.map(([lb,a,h,kind,low,pointLow])=>{
    ...
    const col=barHome?T.home.color:T.away.color;
  ...
  return `<div class="tsp"><div class="tsp-head"><span class="tsp-cap">TEAM</span><span class="tsp-note">bar shows the edge</span></div>${out}</div>`;
}
function controls(){
