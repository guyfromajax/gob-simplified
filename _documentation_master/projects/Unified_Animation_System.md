...
> **Status note (2026-06-12)** — moved from `00_General_Systems/` to `projects/`: this is a point-in-time working spec / migration plan, not target-architecture documentation. Target architecture is [`UESS_System.md`](../00_General_Systems/UESS_System.md) (backend-emitted `animation_steps`, FE as pure renderer).
>
> **Relationship to the UESS pure-renderer rule:** this blueprint intentionally does **not** adhere to it. It governs the hybrid migration regime — backend authority preferred, FE fallback logic (heuristic targets, FE kinematics, contract validation in `unitCompletionContract.js`) allowed as explicit, telemetered degraded mode. The FE contract/telemetry layer it specifies is still live in code (six animation modules import `enforceUnitCompletionContract`). It is retired when [`UESS_Backlog.md`](UESS_Backlog.md) item 13 (FE pure-renderer cleanup, ~46 violations) closes.
>
> The two P0 clock-overrun issues from "Known HCO Turn Issues" (bottom) are now also tracked in [`bugs.md`](bugs.md).
...
- **UESS system source of truth:** `_documentation_master/00_General_Systems/UESS_System.md` holds locked UESS contract cards and cross-layer sync policy.
