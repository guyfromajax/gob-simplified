# Projects

Cross-cutting engineering and product plans that span multiple systems (not tied to a single numbered `_documentation_master` subsystem folder).

## Active

| Document | Topic |
|----------|--------|
| [bugs.md](./bugs.md) | Product bugs, features, animation/UESS backlog |
| [Animation_Cleanup.md](./Animation_Cleanup.md) | FE animation consolidation (Tier 2–5, Projects A–D) |
| [offensive_state_hardening.md](./offensive_state_hardening.md) | `offensive_state` routing hardening (open) |
| [Dynamic_HCT_Turns.md](./Dynamic_HCT_Turns.md) | Dynamic HCT turns |
| [secondary_announce/](./secondary_announce/) | Secondary announce briefs |

## Canonical docs (moved out of projects)

| Was | Now |
|-----|-----|
| Sound Design / SFX Manager / SFX Brief | [`05_Features/SFX_System.md`](../05_Features/SFX_System.md) |
| UESS Legacy Audit | [`00_General_Systems/UESS_Backlog.md`](../00_General_Systems/UESS_Backlog.md) + UESS §12 |
| Movement Rate Refactor (shipped) | [`Z-Completed/Movement_Rate_Refactor.md`](./Z-Completed/Movement_Rate_Refactor.md) |
| Fast Break Refactor (open legacy work) | [`Z-Completed/Fast_Break_Refactor.md`](./Z-Completed/Fast_Break_Refactor.md) + `bugs.md` |
| `Animation_System_Updated.md` | Redirect stub → UESS / Step_By_Step / Advance_Triggers |

## Completed archive

| Document | Topic |
|----------|--------|
| [Z-Completed/Parallel_Franchise_CPU_Sims_and_Week_Finalization_Work_Plan.md](./Z-Completed/Parallel_Franchise_CPU_Sims_and_Week_Finalization_Work_Plan.md) | Background CPU sims, week finalization |
| [Z-Completed/Movement_Rate_Refactor.md](./Z-Completed/Movement_Rate_Refactor.md) | AG-driven timing (shipped May 2026) |
| [Z-Completed/Fast_Break_Refactor.md](./Z-Completed/Fast_Break_Refactor.md) | Legacy FB animation engineering map |
| [Z-Completed/SFX_System.md](./Z-Completed/SFX_System.md) | Superseded by SFX_System |
| [Z-Completed/SFX_Manager_Implementation.md](./Z-Completed/SFX_Manager_Implementation.md) | Superseded by SFX_System |
| [Z-Completed/SFX_Brief.md](./Z-Completed/SFX_Brief.md) | Creative brief archive |

## Website copy

[`Website_Copy/`](./Website_Copy/) — marketing and alpha UI strings.
