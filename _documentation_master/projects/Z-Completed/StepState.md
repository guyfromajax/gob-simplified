...
1. ✅ **DONE (`6cf376532`, 07-13) — wrapper/flag cleanup.** Collapsed the 3 resolver wrappers (`resolve_motion_offense_shot` / `_resolve_motion_offense_shot_dynamic` / `_resolve_setplay_offense_shot_dynamic`) → the ONE `_resolve_hco_offense_shot_dynamic`; deleted the neutered `_dynamic_hco_motion_enabled` + `_dynamic_hco_setplay_enabled`; repointed the 2 production callers + tests + prototype; pruned the flag gate tests. Pure rename + dead-code removal (−73 lines), HCO sim smoke clean.
2. ✅ **DONE (07-13) — bat-OOB follow-ups.** Removed the inert `bat_reach`/step-`bat_oob` handling from `skeleton_step_emitter.py` (`steal_reach` preserved; no writer remained). Closed the **HCT UESS gap**: `dynamic_hct.py` now sends `bat_oob_target = nearest_oob_point(bat_oob_contact)`, so the FE reads the engine-owned exit point (all HCO/HCT/FCP source the OOB exit from the backend). Sim smoke clean.
3. ✅ **DONE (07-13) — render-adoption verified.** No redundant render-side draw: the emit draws the defender grid ONCE and stashes it (`game._hco_render_animations`); `build_step_states` reads that stash for `StepState.defense`; the render never calls `compute_defender_grid`. The contest's pre-emit draws (resolver pre-walk stamp + coverage stamp) are inherent to the circular dependency (contest decides the interception before the emit runs) — same code, ~2px RNG, live GAP already 0%. Nothing to remove.
...
**→ The StepState refactor is COMPLETE.** Stages 1/2/3 done+committed; all residuals + cleanups closed. Only item 4 (optional, deferred, not worth it) remains. Next substantive work is tuning (shot-system) or a fresh thread, not StepState.
...
2. **Stage 2 residual — verify render adoption. ✅ DONE (07-13):** the emit draws the grid ONCE and stashes it (`game._hco_render_animations`); `build_step_states` reads that stash for `StepState.defense`; the render never calls `compute_defender_grid`. No redundant render-side draw. (Contest's pre-emit draws are inherent to the circular dependency.)
