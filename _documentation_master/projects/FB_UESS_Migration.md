...
- **Phase 4 — cutoff cascade.** On `POS_O` the universal `resolve_fb_drive_with_cascade` re-ranks from the shimmy with beaten stoppers excluded (uncapped among on-floor defenders). After-steal also skips x-ahead-invalid meets and tries the next defender. Rim-reaching cascades collapse to one curved `POS_O`; a later stop ends the cascade. Beaten stoppers (`cascade_beaten_stopper_ids`) trail.
