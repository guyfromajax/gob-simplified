...
1. **Stat Categories**:
   - **Standard Basketball Stats**: FGM, FGA, 3PTM, 3PTA, FTM, FTA, PTS, REB, OREB, DREB, AST, STL, BLK, TO, F, PIP, FB_PTS, POT, MIN
...
- season stat rollups write to the actual stored team-play key; runtime still tolerates legacy name-keyed team `plays` maps (renames no longer break aggregation)
- training report payload includes `plays_data` and `plays_effectiveness_changes` (keyed by `play_id` when available); the frontend resolves deltas by `play_id` first, display name as fallback
- putback-miss rebounds are recorded on the **canonical roster player instance** (normalized identity path) — independent of the play-identity migration
...
- **`POT`** (Points Off Turnovers): Incremented (by points scored) for the shooter on **steal-initiated** fast-break makes (`fb_roles["is_steal_entry"]`). Tracked in `shot_manager.py` and `phase_resolution.py`.

**After Steal resolver (stat path note):**
- Steal-entry breaks with the After Steal play short-circuit into `BackEnd/engine/after_steal_fast_break.py`, which resolves its own make/miss and records stats **directly** (F, FGA, FGM via `apply_scoring`, rebound chain on the canonical roster player) instead of going through `shot_manager.resolve_shot`. Team-level per-play FB scouting (`fast_break_plays["after_steal"]`) is still incremented in `phase_resolution`.
- On a made After Steal finish the resolver records **FB_PTS** and **POT** (each `amount=2`; the play is always steal-initiated), and intentionally does **not** record PIP — consistent with the policy above. (Fixed 2026-06-12; it previously recorded PIP only.)
...
- **`fast_break_plays`** (Offense): Per-play **attempts (`A`)** and **successes (`S`)** for `covert_release`, `rim_runner`, `triangle`, `after_steal`; same success rules as **`Fast_Break_Success`**, applied to the play bucket for that possession. **`Fast_Break_Entries`** / **`Fast_Break_Success`** are incremented in parallel (both aggregate and per-play rows). Defense has **no** per-play FB scouting.
