...
**National Rank and Prestige System** (**verified 2026-06-13**)

> Verified vs `BackEnd/utils/franchise_rank_prestige.py` + wiring in `franchise_routes.py` — **fully implemented; all formulas match exactly.** Prestige delta tiers (8/6, 9/8, 10/10, 13/12, 18/15), weeks-1-4 `2×` multiplier (`_prestige_delta_multiplier`), floor/ceiling dampeners (`(loser-200)/100`, `(800-winner)/100`), ranking-score phases (preseason `*0.10`; wk1-4 attr taper `0.04/0.03/0.02/0.01` + `100*wins`; wk5-8 `0.75*prestige`+`100*wins`; wk9-12 `0.5`+`80`; wk13-26 `0.25`+`60`; `sos_score=(129-sos_avg)*4`), and sort/tiebreak logic all confirmed. Constants `FRANCHISE_RANK_PRESTIGE_SYSTEM_VERSION=2`, `SOS_AVG_DEFAULT=64.0`, `PRESTIGE_FLOOR=200`, `PRESTIGE_CEILING=800`. SOS accumulators (`sos_rank_sum`/`sos_games_played`), v2 gate (`use_franchise_rank_prestige_v2`), and idempotency field `rank_prestige_last_applied_week` all wired in `franchise_routes.py`.
>
> **Pseudocode vs actual symbols:** the code's real names are `calculate_prestige_delta(winner_prestige, loser_prestige, week=None)` and `calculate_ranking_score(prestige, total_player_attrs, week, team_wins=0, sos_avg=64.0)` (note: `week`/`team_wins` are real params), the ranking function is **`rank_teams_for_week`** (not `generate_national_rankings`), and `core_total_player_attrs` sums the 12 core attrs. The blocks below are illustrative spec, not verbatim source.
...
- `BackEnd/utils/franchise_rank_prestige.py` — shared rank/prestige utility functions (`calculate_prestige_delta`, `apply_prestige_delta`, `calculate_ranking_score`, `rank_teams_for_week`, `use_franchise_rank_prestige_v2`, `core_total_player_attrs` + constants)
- Weekly application + SOS accumulators + idempotency marker (`rank_prestige_last_applied_week`) live in `BackEnd/api/franchise_routes.py` (`use_franchise_rank_prestige_v2`-gated block)
