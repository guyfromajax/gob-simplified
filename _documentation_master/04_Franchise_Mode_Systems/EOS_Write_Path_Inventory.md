...
Last reviewed: **verified 2026-06-13** — every function/endpoint/script in the tables confirmed present: `_eos_calendar_advance_update_fields` (L4002), `_finalize_franchise_week_after_cpu_games` (L4087), `_eos_advance_all_conference_brackets_until_idle` (L4375), `_eos_heal_conference_eos_from_games` (L4467), `_training_status_reset_after_advance_to_week` (L121), `_sync_eos_bracket_from_existing_game_doc` (L3837), endpoints `/franchise/sim-rest-of-tournament` (L11740) & `/franchise/sim-championship` (L11963); `record_tournament_game_result` (L258) / `advance_conference_bracket` (L240) / `advance_national_bracket` (L248) in `franchise_tournament_progression.py`; `get_eos_week_games` in `franchise_tournament.py` (L432); `repair_franchise_eos_bracket_from_results` (`repair_franchise_eos_bracket.py` L133) + `scripts/repair_franchise_eos_bracket_from_results.py`. (Line numbers approximate — drift over time.)
