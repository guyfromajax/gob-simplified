...
3. **Possession Flip Locations**:
   - **SIP Transitions**: `game_manager.py` `simulate_macro_turn()` flips BEFORE `setup_side_inbound()` (line ~403)
   - **DREB → HCO**: Flips before HCO turn creation (line ~311)
   - **DREB → Fast Break**: Flips before Fast Break turn creation (line ~322)
   - **OREB Transitions**: Flips after OREB turn creation (line ~292)
   - **Made Shot → BIP**: Flips before BASELINE_INBOUND turn creation (line ~491)
4. **Key Files**:
...
1. **SIP Transitions** (`game_manager.py` `simulate_macro_turn()` line ~403):
...
2. **DREB → HCO** (`game_manager.py` `simulate_macro_turn()` line ~311):
   ...
   - Updates `offense_team_id` AFTER flip to reflect new offense team

3. **DREB → Fast Break** (`game_manager.py` `simulate_macro_turn()` line ~322):
   - Flips before Fast Break turn creation
   - Handles: MISS with DREB → Fast Break, STEAL → Fast Break
   - Updates `offense_team_id` AFTER flip to reflect new offense team

4. **OREB Transitions** (`game_manager.py` `simulate_macro_turn()` line ~292):
   - Flips after OREB turn creation
   - Handles offensive rebound putback scenarios
   - Clears flag after flipping

5. **Made Shot → BIP** (`game_manager.py` `simulate_macro_turn()` line ~491):
...
- Location: `FrontEnd/static/js/phaser/animation/turnPreparation.js` line ~157
...
**Fix 2: DREB → HCO Possession Flip (January 2025)**
- **Issue:** `offense_team_id` was set before flip, causing incorrect value
- **Solution:** Flip possession first, then update `offense_team_id` AFTER flip
- **Result:** HCO turn now correctly has `offense_team_id` set to new offense team

**Fix 3: DREB → Fast Break Possession Flip (January 2025)**
- **Issue:** Same as Fix 2 - `offense_team_id` set before flip
- **Solution:** Flip possession first, then update `offense_team_id` AFTER flip
- **Result:** Fast Break turn now correctly has `offense_team_id` set to new offense team
...
**Backend:**
- `BackEnd/models/game_manager.py` - Possession management and flip logic
  - `simulate_macro_turn()` (lines 249-600) - Main turn simulation, handles possession flips
  - `switch_possession()` (line 750) - Swaps offense and defense teams
- `BackEnd/models/turn_manager.py` - Sets `offense_team_id` in turn results
...
- `FrontEnd/static/js/phaser/animation/turnPreparation.js` - Frontend possession handling
  - `handleTurnTransition()` (lines 144-179) - Reads `offense_team_id` and updates scene state
- `FrontEnd/static/js/phaser/utils/possessionManager.js` - Possession change event handling
...
- `FrontEnd/static/js/phaser/animation/animateGameTurns.js` - Possession change event listener
  - `handlePossessionFlip()` (lines 515-545) - Handles possession change events during animation
