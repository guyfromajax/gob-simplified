...
| `franchise_players_data` (FPD) | Per-franchise player docs: meta, attributes, position ratings, season/career stats, `titles` (`conf_rs` / `conf_t` / `region` / `national` ints; incremented on the user team's active roster when a title is awarded; future-forward, no backfill), and the top-level identity/development carry — `entry_tier`, `position_intent`, `potential_factor`, plus the `development` subdoc (`peak_count`, `peak_rungs`, `family_timing`, `ch_seed`, `ht_total`) and `training_position`/`training_focus`/`coaching_quality` (see `PLAYER_DEV_CARRY_FIELDS`; `training_position` and `training_focus` are coach-editable via Development Focus) |
