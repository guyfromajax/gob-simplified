...
- Displays all team attributes with visualizations:
  - **Red/Green Pills:** Most attributes (Shooting, Rebounding, Offense, Defense, Fast Breaks, Press/Trap, Fight, Discipline, Momentum)
    ...
    - **Shooting (`shot_threshold`):** Golf-score attribute (lower raw value is better). Pill centers at **110** with span **10–210** (better toward the right). See [Shot_Threshold_Scale_Tuning.md](../04_Franchise_Mode_Systems/Shot_Threshold_Scale_Tuning.md). The **numeric change** next to the label uses **inverted sign** versus the raw delta: raw **−10** displays as **+10** (green); raw **+5** displays as **−5** (red).
