...
Feed for the **Community Highlights** panel on **Mode Select** (`FrontEnd/static/mode-select.html`). **Implemented** (verified 2026-06-13): markup `section.community-highlights-section` → `.community-highlights-panel` → `.community-highlights-body` is populated by `renderCommunityHighlights(data)` in `mode-select.js` (the old “Coming Soon” placeholder is gone).
