...
`generate_walk_on_profile()` — `BackEnd/models/franchise_manager.py:155`. Shared by season-1
...
- **Season 1:** `initialize_season` generates **3 walk-ons per team (all 128)** during
  franchise creation (under the load screen), → 15-man rosters. `franchise_manager.py:335`.
  FTD `training_squad_players` starts `[]` (assigned at camp). `franchise_manager.py:580`.
...
- **User** — `POST /franchise/cut-players` → `cut_franchise_players` (`franchise_routes.py:9372`).
  Requires exactly 3 → TS, leaving 12 active. Drives the cut-players page in its **default
  (assignment) mode**.
- **CPU** — `_apply_cpu_training_camp_cuts` (`franchise_routes.py:8177`): lowest-RT 3 → TS.
...
- **Requirement signal** — `_week_1_cut_requirement` (`franchise_routes.py:8153`): `cut_count =
...
`_apply_training_squad_progression_and_report()` — `franchise_routes.py:8263`, called once per
completed week from `_finalize_franchise_week_after_cpu_games` (`franchise_routes.py:4038`).
...
- Per-week delta band is **CH-gated** (`_ts_progression_band`, `franchise_routes.py:8239`):
...
- Served by `GET /franchise/training-squad-reports` (`franchise_routes.py:9569`), newest week first.
...
`franchise-command-center.js:3811`).
...
- **Trigger:** week-35 "Run Recruiting" button shows a modal — *"Would you like to cut any
  players ahead of recruiting?…"* / **Cut Players** | **No Cuts** (`franchise-command-center.js:3518`).
...
- **User cut page (`mode=cut`)** — `cut-players.js` `isCutMode`: shows **active + training squad**,
  ...
  (`cut_franchise_players_final`, `franchise_routes.py:9533`) → `_hard_release_players`
  (`franchise_routes.py:9455`) deletes FPD + strips all FTD lists.
- **CPU cuts** — `_apply_cpu_week_35_cuts` (`franchise_routes.py:9493`), run inside
  `run_week_35_recruiting` (`franchise_routes.py:9027`) **before** `_run_week_35_signings`,
...
In the rollover (`franchise_routes.py:11666`, `returning_candidate_ids`), both
...
the cap: `_current_team_capacity_state` (`franchise_routes.py:8647`) and
...
- **FCC Roster tab** — `renderTrainingSquad` (`franchise-command-center.js:2178`) renders a
...
| Area | File:symbol |
|---|---|
| Walk-on generator | `BackEnd/models/franchise_manager.py:155` `generate_walk_on_profile` |
| Season-1 walk-ons | `BackEnd/models/franchise_manager.py:335` (in `initialize_season`) |
| Progression + report | `BackEnd/api/franchise_routes.py:8263` `_apply_training_squad_progression_and_report` |
| CH band | `BackEnd/api/franchise_routes.py:8239` `_ts_progression_band` |
| Week-completion hook | `BackEnd/api/franchise_routes.py:4038` `_finalize_franchise_week_after_cpu_games` |
| Camp assignment (user) | `BackEnd/api/franchise_routes.py:9372` `cut_franchise_players` |
| Camp assignment (CPU) | `BackEnd/api/franchise_routes.py:8177` `_apply_cpu_training_camp_cuts` |
| Real cut helper | `BackEnd/api/franchise_routes.py:9455` `_hard_release_players` |
| Week-35 user cut | `BackEnd/api/franchise_routes.py:9533` `cut_franchise_players_final` |
| Week-35 CPU cut | `BackEnd/api/franchise_routes.py:9493` `_apply_cpu_week_35_cuts` |
| Report endpoint | `BackEnd/api/franchise_routes.py:9569` `get_training_squad_reports` |
| Rollover fold-back | `BackEnd/api/franchise_routes.py:11666` `returning_candidate_ids` |
| Capacity (15-cap) | `BackEnd/api/franchise_routes.py:8647` `_current_team_capacity_state` |
| Roster `training_squad` | `BackEnd/api/api.py:5530` (in `get_team_roster`) |
| FCC TS display | `FrontEnd/static/franchise-command-center.js:2178` `renderTrainingSquad` |
| Cut page (dual-mode) | `FrontEnd/static/cut-players.js` (`isCutMode`), `cut-players.html` |
| Week-35 cut modal | `FrontEnd/static/franchise-command-center.js:3518` |
| Report page | `FrontEnd/static/training-squad-report.html` / `training-squad-report.js` |
| Inbox link | `FrontEnd/static/franchise-command-center.js:3811` (`renderFccInbox`) |
