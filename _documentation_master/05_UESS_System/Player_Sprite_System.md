...
Field source: `player.height` (integer inches). See `../05_GP_Supporting_Systems/Game_Init_System.md` → "Player-level data ingestion" for the load path (DB → `Player.__init__` → `summarize_game_state` → simData).
...
| Away | **white** (`0xffffff`) | team **primary** |

On the v2 sprite the tile sits in front of the team-color backdrop disc (home backdrop = primary; away = soft white `0xf5f5f5`). Home's tile and backdrop are both primary by design — they blend into a single field while the secondary-color initials remain readable. Away's white tile sits over the soft-white backdrop for a clean white field that the primary-color initials read against.
...
| `height` | number \| null | **Integer inches.** Wired through `summarize_game_state` and the `/api/game` projection (May 2026). Used by v2 height-linked radius. See `../05_GP_Supporting_Systems/Game_Init_System.md` → "Player-level data ingestion" for the full DB → simData load path. |
...
If a marker feature requires a field not on this list, propagate it through `summarize_game_state` and the `/api/game` `players_with_energy` projections — see the recipe in `../05_GP_Supporting_Systems/Game_Init_System.md`.
