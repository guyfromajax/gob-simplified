...
7. **Fast break initiation (single roll)**: Slider **0–4** maps to probability **`{0: 0.0, 1: 0.25, 2: 0.5, 3: 0.75, 4: 1.0}`** via `fast_break_probability_from_slider()` in `BackEnd/utils/shared.py` (`SLIDER_TO_FAST_BREAK_PROB`). **Miss → DREB** sources share one arming helper (`BackEnd/engine/dreb_fast_break_arming.py` → `arm_dreb_fast_break`): **HCO shot**, **final FT miss**, **OREB putback miss**, and **FB shot miss** (chained FB allowed). One roll on the **rebounding team’s** `strategy_settings["fast_breaks"]` only — no second roll for Covert release. Situational Force Foul after DREB still forgoes FB. **Steal:** one roll on the **stealing team’s** `strategy_settings["aggression"]` only (that team is `def_team` at steal resolution before possession flips in `game_manager`). User playbook settings now also seed **`playbook_settings.fast_break`** with `covert_release = 50`, `rim_runner = 50`, and `triangle = 0`.

8. **EOQ entry contract**: At `0 < time_remaining <= 30`, Fast Break shares the universal Q4/OT priority with HCO/HCT/FCP: Force Foul → Run Out → Quick Shot → Final Shot. Run Out overrides the break. On a Final-Shot path, the selected migrated FB resolver is previewed on an RNG-neutral clone. A complete fitting break runs normally; an overrun contributes only complete non-terminal schema steps within `time_remaining - 1`, then FLSS starts from the resulting live ball handler and coordinates. Speculative preview score/stat/foul/turnover/rebound/possession mutations never reach the live game. The fixed `≤8s` FLSS rule is only a fallback when no usable preview schema exists.

**Covert Release prep by DREB source** (full play mix RR/CR/Triangle on all sources):

| Source | Covert prep |
|--------|-------------|
| **HCO miss** | Existing `shot_manager` release + getback selection / sampled coords |
| **OREB putback → DREB** | **Carry** original HCO miss `defense_release` / `offense_getback` (+ coords); re-arm `last_release_player` + pending play key — **no resample** |
| **Final FT miss → DREB** | Geo: release = FT-defense nearest center court; getbacks = FT-offense nearest center (`FT_DREB_FB_GETBACK_COUNT`, default 1; geo-ready for 2) |
| **FB miss → DREB** | Outlet/release = rebounding-team nearest center; if that player is the rebounder → **skip outlet / dribble** (RR pattern). Getbacks = up to 2 from the team that just missed who are closer to the **new** FB attack basket than the outlet receiver; else 0 getbacks |
...
3. **Determine Entry Type and Set Roles**
   - **DREB Entry (shot turn / shared arming)**: On the miss that produces DREB, **one** roll via `arm_dreb_fast_break` / HCO `shot_manager` path: `random.random() < fast_break_probability_from_slider(rebounding_team["fast_breaks"])`. **No second roll** for Covert. If eligible, **`play_key_for_fast_break_entry(True)`** runs **once** (playbook weights; default mix includes `rim_runner` / `covert_release` / `triangle`). **Covert Release** prep is **source-specific** (HCO selection, OREB carry, FT geo, FB-miss geo — see initiation table above). **Rim Runner / `triangle`**: all defenders crash; no release list. Key stored as **`game_state["pending_dreb_fb_play_key"]`** when miss → DREB and **`next_play_type == "FAST_BREAK"`** (consumed in resolver).
...
- **Lane-pass shot continuity**: The pass arrival interrupts every non-gate runner at a reachable `end.coords`, but does not cancel that runner's intent. Any unfinished lane-pass destination carries into the shot-motion step: offense continues as `cut`, defense as `guard_offball`, using the carried archetype and interrupted-coordinate math for the shot-step duration. The shooter instead targets the authoritative `shot_spot`; a designated shot defender instead targets the deterministic geo-contest coordinate. This prevents trailing defenders or off-ball runners from parking while the RR completes the shot.
...
- **Resolution (high level)**: Dedicated module **`resolve_rim_runner_fast_break()`** — build **`rim_runner_burst_phase`** + outlet roles → outlet contest → on success → **geo lane-threat count** (`fb_open`) → ball-handler IQ read → pass vs hold → if pass and open: FB shot path; if pass and not open → intercept tiers / bat OOB (`rim_runner_bat_oob`, SIP) / completion to shot. Uses team attrs **`fb_efficiency`** / **`fb_opp_modifier`** (clamped **−10…+10**) and `random.randint(1,6)` where applicable.
...
- **Open lane (`fb_open`, geo)**: Count defenders who threaten the BH→RR lane pass using the **same gate** as steal/bat resolve (perp ≤ **8** to BH→RR segment **and** defender at/past RR on x). Default: **`fb_open`** if count **≤ 1** (hold correct when **> 1**). If offense **`strategy_calls.aggression_call == "passive"`**: **`fb_open`** only if count **≤ 0** (hold correct when **> 0**). Count stored as `rim_runner_lane_threat_count`. Shared by Rim Runner and Triangle.
- **Read**: `(IQ + fb_efficiency) * d6` vs threshold **`200 - 5×fb_efficiency`** (offense team). Misread pass/hold bias uses offense **`strategy_settings.aggression`** (≥3 = aggressive ≈67% still attempt; else 50/50) — distinct from Stage C’s `aggression_call`.
- **Pass / shot / events**: Open lane → FB shot (Rim Runner shooter, existing `resolve_shot` attack path). Forced pass when not open: intercept roll vs **`250 - fb_opp_modifier`** / **`200 - fb_opp_modifier`** tiers; bat OOB sets `rim_runner_bat_oob`, animates the deflection, keeps contact SFX, and is visually silent (no announcement banner). Full detail matches `rim_runner_fast_break.py`.
...
- **RR archetype in the lane-pass step (universal — RR + Triangle)**: The `burst` archetype (`BURST_GRID_PER_GAME_SEC` = 32 g/s) is the **peak explosive start only** — Step 0's fixed one-game-second advance. It must **not** carry into the lane pass / receive-and-drive or the RR jets to the rim. The shared `_build_lane_pass_step` (`rim_runner_step_emitter.py`) caps the RR at **`sprint`** via `_rr_post_burst_archetype`. This closes a gap on **`skip_outlet_pass`** turns (outlet receiver already holds the ball): those turns bypass the outlet-pass step's `rr_archetype_override="sprint"`, which previously let the `burst` payload leak straight into the lane pass (~32 g/s jet across burst + pass). Regression: `test_lane_pass_step_decelerates_rr_burst_to_sprint`.
- **RR read gate**: After a successful outlet, Triangle still performs the RR lane-pass read first. It uses the **same** geo `fb_open` / `correct_read` / `pass_attempted` Stages B–E as Rim Runner (no separate Triangle burst multiplier).
...
- **Cutoff cascade** (universal helper `resolve_fb_drive_with_cascade` in `fb_drive_resolution.py`; after-steal is the first caller): if the BH beats his cutoff (`POS_O`), remaining defenders are **re-ranked** from the shimmy (path changes) with the beaten stopper in `excluded_stopper_ids`. Continues until a stopping outcome or no eligible defenders remain (`FB_AS_MAX_CUTOFF_ATTEMPTS = None`). On after-steal, a meet that fails the **x-ahead** filter skips that defender and tries the next-soonest in the same drive step (does not clear the lane by itself). Cascades that reach the rim collapse into one curved `POS_O` drive; a later defender who stops the BH ends the cascade with his own outcome. Beaten stoppers (`cascade_beaten_stopper_ids`) trail the play.
...
- `BackEnd/engine/dreb_fast_break_arming.py` — Shared DREB→FB arming (`arm_dreb_fast_break`) + Covert prep adapters (OREB carry / FT geo / FB-miss geo)
- `BackEnd/constants/fast_break_play_types.py` — Play keys, `default_fast_break_plays()`, `ensure_fast_break_plays()`, `play_key_for_fast_break_entry()` (DREB: playbook weights; live path uses **`pending_dreb_fb_play_key`**)
...
- `BackEnd/models/shot_manager.py`
  - **DREB FB**: `arm_dreb_fast_break` (`dreb_fast_break_arming.py`) + HCO `shot_manager` path — one `fast_breaks` roll → play key → source-specific Covert prep or all-crash for RR/Triangle; **`pending_dreb_fb_play_key`** on DREB → FAST_BREAK
...
**Make-time `offensive_state` reset invariant *(all FB keys)*:** On a **made** FB finish (non-and-1) the resolver **must** clear `game_state["offensive_state"]` off `"FAST_BREAK"` — it sets it to `game.turn_manager.determine_defensive_pressure_type()` (fallback `"HCO"`) and stamps that value as `next_defensive_setup` on the turn result. `main.py` reads `offensive_state` at the flipped **BASELINE_INBOUND** to pick the next possession type, so a stale `"FAST_BREAK"` would incorrectly route the *new* offense into another fast break after a make. After-steal (`after_steal_drive_integration.py`) and covert-release (`covert_release_drive_integration.py`) already did this; RR/Triangle (`rim_runner_drive_integration.py`) previously reset only on stop/foul/terminal branches — **both** make branches (POS_O rim finish **and** the NEUTRAL meet-path shoot/pass make) now reset it too. And-1 makes intentionally do **not** reset here (they route to `FREE_THROW`), mirroring the sibling integrations. Regression tests: `test_pos_o_make_resets_offensive_state_off_fast_break`, `test_neutral_shoot_make_resets_offensive_state_off_fast_break` (`tests/test_rr_triangle_drive_resolution.py`).

**Terminal foul / turnover freeze *(all FB keys)*:** FB terminal turns that resolve to a **charge, non-shooting foul, or dead-ball turnover** freeze via a backend-stamped blocking schema announcement (`BackEnd/engine/fb_terminal_announce.py` → `stamp_fb_terminal_freeze`, called at the tail of each drive-resolution emitter). The hold uses `ANNOUNCEMENT_FREEZE_HOLD_MS` (currently **300ms**). The frontend renders it and pauses — no FE decision. The matching legacy FE turn-end callout is suppressed via `suppress_turn_prep_foul_announce` / `suppress_turn_prep_turnover_announce`. Shooting fouls/and-1 and batted-OOB are out of scope. Foul flavor language + random dead-ball text were ported from the frontend to the backend. Dead-ball fumble injection runs before `stamp_fb_terminal_freeze`; when the fumble step is inserted, it owns the Travel/Double Dribble headline and the generic FB terminal turnover announcement no-ops. See **Announcement_System.md §Fast Break foul / turnover freeze**.

**Dead-ball fumble schema anchor *(RR/Triangle drive terminal path)*:** Terminal RR/Triangle cutoff-drive outcomes that resolve as `result_type == "DEAD BALL"` / `"TURNOVER"` must end the meet step with `next: {"kind": "turn_stop", "event": "DEAD_BALL_TURNOVER"}` rather than generic `end_of_turn`. `dead_ball_fumble.py` keys off that turn-stop anchor to insert the fumble micro-animation before the whistle headline. Batted-OOB remains excluded via `rim_runner_bat_oob` because the offense retains possession and the OOB beat owns that animation.
