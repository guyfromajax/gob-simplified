...
**Terminal foul / turnover freeze *(all FB keys)*:** FB terminal turns that resolve to a **charge, non-shooting foul, or dead-ball turnover** now FREEZE for 1000ms via a backend-stamped blocking schema announcement (`BackEnd/engine/fb_terminal_announce.py` → `stamp_fb_terminal_freeze`, called at the tail of each drive-resolution emitter). The frontend renders it and pauses — no FE decision. The matching legacy FE turn-end callout is suppressed via `suppress_turn_prep_foul_announce` / `suppress_turn_prep_turnover_announce`. Shooting fouls/and-1 and batted-OOB are out of scope. Foul flavor language + random dead-ball text were ported from the frontend to the backend. See **Announcement_System.md §Fast Break foul / turnover freeze**.
