...
- The first team to take possession of the ball with ≤ 30 seconds remaining will hold for the final shot, so time_elapsed will be equal to time remaining.
  ...
  - **Play Execution:**
    ...
    - Shooter will be chosen at random:
      ...
      - If the shooter is the ball handler, he dribbles to that wing location. If he's not the ball handler, he moves to his location, then the ball handler passes the ball to him.
        ...
        - **Frontend pacing gate (current implementation):** After Final Turn alignment, the ball handler holds until about 6 seconds remain (small jitter window), then initiates the pass/shot sequence. If the turn starts with `<= 6` seconds remaining, the pass begins immediately.
  ...
  - **Alignment (backend-owned):** The backend emits `oDestinations` and `dDestinations` in final display orientation. Home offense uses the home-authored Final Turn templates; away offense mirrors both maps with `x_away = 100 - x_home` before emission so both teams set up on the away attacking half. The frontend renders these coordinates directly and does not infer or apply orientation.
