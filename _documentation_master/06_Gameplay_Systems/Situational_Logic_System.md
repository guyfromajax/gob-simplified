  ...
  - **Play Execution:**
    ...
    - Shooter will be chosen at random:
      ...
      - If the shooter is the ball handler, he dribbles to that wing location. If he's not the ball handler, he moves to his location, then the ball handler passes the ball to him.
        ...
        - **UESS schema playback (alignment + pacing):** Final Turn shot attempts render through the same pure-renderer path as HCO — the frontend plays the full backend ``animation_steps[]`` payload from step 0 through shot resolution with **no** parallel alignment tween, step-0 skip, coord patch, or imperative clock-hold hooks. **Step 0 (alignment)** moves all ten players to Final Turn starting spots using backend **archetypes** and ``tween_durations`` (not FE agility tweens). The ball stays with the **live prior-turn owner** through step 0. After a normal SIP, that owner is **PG** (SIP inbound pass is **SF→PG** on the SIP turn itself — SF is the passer, not the holder entering Final Shot). ``SITUATIONAL_SIP_RECEIVER_POS`` (SG) is only the Force Foul foul-target on SIP, not the inbound receiver. When the live owner differs from the Final Turn skeleton ball handler (e.g. PG holds after SIP but the skeleton drew SF/SG as BH), the emitter inserts a universal ``build_pass_step`` entry pass at **24 grid/game-sec** before skeleton step 1; when they match (common: PG after SIP + PG as BH), no entry pass is inserted. The backend stamps ``_step_t_floor_game_seconds`` on skeleton step 0 so the game clock burns down until **~3s remaining (Outside)** or **~4s remaining (Attack)** before steps 1+ (pass / drive / shoot) fire; if the turn starts with time already at or below that target, the floor is omitted and the pass/shot sequence begins immediately. When the ball handler is not the shooter, step 1 pass/receive transfers the ball on-schema. The turn's ``clock_start`` → ``clock_end`` contract and ``real_time_elapsed_ms`` still drive scoreboard interpolation via ``AnimationRouter`` in parallel with schema playback. Config mirrors: ``animation_config.js`` → ``finalTurn.latePassTargetSecOutside`` (3), ``finalTurn.latePassTargetSecAttack`` (4) — authoritative pacing is backend step 0 floor + emitter step T.
