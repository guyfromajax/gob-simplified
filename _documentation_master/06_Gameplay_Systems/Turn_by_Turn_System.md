...
- `dreb_outlet_pass` - Outlet contract for **MISS/BLOCK → DREB → {HCO|HCT|FCP}** (strict shape may include `receiver_target`; see `projects/Unified_Animation_System.md`). Authored on the **MISS/BLOCK** turn and consumed by the **next HCO turn's schema entry orchestrator**, which renders the outlet/handoff. (The old path — a frontend `runDefensiveReboundSetup` after the discrete DREB row — was removed to prevent double outlet execution.)
...
**Related docs:** `projects/Unified_Animation_System.md` (execution unit **`hco.lead_in.from_dreb_outlet`**, strict `dreb_outlet_pass` contract), `Announcement_System.md` (**Rebound!** headline vs outlet choreography), `Rebound_System.md` (post-shot placement vs outlet).
