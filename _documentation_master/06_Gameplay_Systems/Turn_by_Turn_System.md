# Turn-by-Turn System (Feb 2025 + UESS additions; **verified + fixed 2026-06-13**)

> **Note (2026-06-13):** Three-bucket structure, discrete-`DREB` turn model, coords-sync funnel, and resolution overview all verified against code. **Fixed:** the doc described a now-**removed** post-DREB frontend outlet lead-in — for the discrete DREB row, `runDefensiveReboundSetup`/`_maybeRunDiscreteDrebOutletLeadIn` is no longer called after the schema DREB turn (it double-executed against the schema handoff). The half-court outlet/handoff/walk-up is now owned by the **next HCO turn's schema entry orchestrator**. (`runDefensiveReboundSetup` is still used for the non-discrete free-throw-DREB and Fast-Break-DREB shot-turn paths.) Line numbers refreshed/softened.
...
- `BackEnd/models/game_manager.py` - `_append_turn()` (single funnel for appending turns + **player coords sync**); `_repair_miss_bounce_rebound_contract()` (MISS/BLOCK bounce + rebound invariant before append); discrete **`DREB`** turn append + `_build_dreb_turn_from_miss()` when migration gates match
...
- `FrontEnd/static/js/phaser/animation/AnimationEngine.js` - Routes **`DREB`** + `animation_steps` through `playTurn` (schema playback). The legacy post-DREB `runDefensiveReboundSetup` lead-in (`_maybeRunDiscreteDrebOutletLeadIn`) was **removed**; the half-court outlet/handoff is owned by the next HCO turn's schema entry orchestrator.
- `FrontEnd/static/js/phaser/animation/turnAnimation.js` - `runDefensiveReboundSetup` (legacy outlet renderer, carries unit `hco.lead_in.from_dreb_outlet`; still used by the **free-throw-DREB** and **Fast-Break-DREB shot-turn** paths, **not** the discrete DREB row)
...
**Set by:** `turn_manager.py` `run_micro_turn()` (~L1122) - Added to ALL results after phase resolution
...
- `dreb_outlet_pass` - Outlet contract for **MISS/BLOCK → DREB → {HCO|HCT|FCP}** (strict shape may include `receiver_target`; see [`Rebound_System.md`](Rebound_System.md)). Authored on the **MISS/BLOCK** turn and consumed by the **next HCO turn's schema entry orchestrator**, which renders the outlet/handoff. (The old path — a frontend `runDefensiveReboundSetup` after the discrete DREB row — was removed to prevent double outlet execution.)
...
2. **`simulate_macro_turn` rebound contract:** before `_append_turn`, `_repair_miss_bounce_rebound_contract()` asserts that any migrated **MISS/BLOCK** row with `ball_bounce_x/y` also has `rebounderId` + `rebound_type`. If those fields were stripped but `game_state.last_rebounder` / `last_rebound` are still set, the repair restores them so discrete DREB promotion and the next HCO entry orchestrator do not see a loose-ball prior turn. Skipped when `quarter_ends_after` is true (intentional EOQ terminal strip).
3. The **`DREB`** turn carries **`animation_steps`** built by `BackEnd/engine/dreb_step_emitter.py` — **rebound capture only** (rebounder to ball; others stationary at post-shot coords from `shot_manager`).
4. **Over The Back on defensive rebounds** is resolved inside `_build_dreb_turn_from_miss()` after the MISS/BLOCK row has synced post-shot coords. When it fires, the DREB row becomes `current_turn: "DREB"`, `result_type: "FOUL"`, `otb_foul: true`; the schema step still animates the rebounder to the loose ball and attaches possession, then emits `turn_stop: FOUL` plus the `"Over The Back!"` announcement. The standard non-shooting foul resolver owns bonus/SIP routing and possession flips.
5. **Half-court outlet** (rebounder → PG / contract receiver, then bring-up toward the new offense end) is **not** inside those `animation_steps`, and is **no longer** run as a post-DREB frontend lead-in. The legacy `runDefensiveReboundSetup` / `_maybeRunDiscreteDrebOutletLeadIn` call after the discrete DREB `playTurn` was **removed** — it double-executed against the schema handoff (legacy outlet pass, then the schema handoff teleporting players back). The BH→PG handoff + walk-up is now owned by the **next HCO turn's schema entry orchestrator**, using the prior MISS/BLOCK row's `dreb_outlet_pass` contract. (The `_maybeRunDiscreteDrebOutletLeadIn` method is retained but unused on the schema path.) This handoff does not apply when the DREB routes to **`FAST_BREAK`** (Fast Break renders its own outlet), when DREB resolves to **`FOUL`**, or when the shot turn has **`force_foul_after_dreb`**.

**Related docs:** [`Core_Animation_System.md`](../05_UESS_System/Core_Animation_System.md) (schema completion authority vs the legacy unit validator), `Announcement_System.md` (**Rebound!** headline vs outlet choreography), `Rebound_System.md` (post-shot placement and the `dreb_outlet_pass` handoff contract).
...
**Set by:** `Animator` class (`animator.py`) - Created in `turn_manager.py` `run_micro_turn()` (legacy `animations[]` path)
...
- `FrontEnd/static/js/phaser/gameScene.js` `simulateTurnByTurn()` method
- Early break only if `!turnData.turn`
- Quarter completion check after animation
...
**For detailed resolution logic, see:** `HCO_Turn_Resolution_System.md`
...
**Backend:**
- `BackEnd/models/turn_manager.py` - Standard fields (Bucket 1), turn creation (`run_micro_turn()` ~L1122)
- `BackEnd/models/shot_manager.py` - Shot-specific fields (Bucket 2)
- `BackEnd/engine/phase_resolution.py` - FCP/HCT/Free Throw fields (Bucket 2), resolution calculations
- `BackEnd/models/animator.py` - Animation data creation (Bucket 3, legacy `animations[]` path)

**Frontend:**
- `FrontEnd/static/js/phaser/gameScene.js` - `simulateTurnByTurn()` method

**Reference Documentation:**
- `docs/GP_Core_Docs/TURN_SYSTEM.md` - Complete turn data structure and execution patterns reference (legacy `docs/` tree)
- `docs/Data_Docs/UNIFIED_DATA_STRUCTURE_ANALYSIS.md` - Analysis of data structure patterns (legacy `docs/` tree)
- `HCO_Turn_Resolution_System.md` - Detailed HCO resolution logic
