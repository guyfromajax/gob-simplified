...
A "box score" in GOB is the per-player stat lines plus team totals for a single game. This doc covers how box scores are produced by the authoritative engine, persisted, consumed by the End of Game (EOG) process, and displayed.
...
During franchise week completion, every CPU game uses the real engine.
...
| Field | Notes |
|---|---|
| `teams[team_id].box_score` | **Canonical.** Position-keyed player rows written by `summarize_game_state`. |
| `teams[team_id].totals` | Team totals for that side. |
| `box_score` (top level, keyed by team_id) | Readers (`finalize_game`, `GET /api/game/{id}`) check top level first and **fall back to building it from `teams[*].box_score`**. |
...
3. **Phase B** — after the Post-Game Press Conference, `/franchise/complete-week/phase-b` runs the rest of the week's slate as full CPU sims, each one **upserted + finalized + team-attrs immediately**, then week-level wrap-up in `_finalize_franchise_week_after_cpu_games` (standings, national ranks, recruiting leans, training-squad gains, weekly news) and the week advance.
...
- Loaded with `game_id` (+ `mode`, `franchise_id`, team names/ids) as URL params; works identically for user and CPU-full games.
...
| | User game (A) | Full CPU sim (B) |
|---|---|---|
| Engine | Real (`GameManager`), turn by turn | Real (`run_simulation`), whole game |
| Box score source | Accumulated per turn | Accumulated per sim |
| Saved when | Every quarter (+ per-turn `$inc`s) | Once, at week completion |
| `simulation_engine` | `turn_by_turn` / `full_quarter_sim` | *absent* (only the simulate-quarter endpoint sets it) |
| Scouting / play `game_stats` | Yes | Yes |
| EOG efficiency attr rules | Scouting-driven | Scouting-driven |
| FPD season/career rollup | Yes (`finalize_game`) | Yes |
...
| EOG orchestration | `BackEnd/api/franchise_routes.py` (phase A/B, `_persist_franchise_user_game_snapshot`, `_finalize_team_attributes_for_game`, `update_team_attributes_after_game`) |
