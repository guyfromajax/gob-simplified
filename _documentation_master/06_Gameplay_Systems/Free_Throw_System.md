...
4. **Secondary Check (Miss-to-Make Conversion)**: a `1–100` roll vs a threshold to convert miss to make. Base %: **Home shooter** `FREE_THROW_MISS_TO_MAKE_SECOND_CHANCE = 0.40`; **Away shooter** reduced by the home crowd factor (`effective_ft_miss_to_make_second_chance`): factor ≤1 → 0.40, factor 2–3 → **0.30**, factor ≥4 → **0.20** (see `Home_Crowd_System.md`). The base is then **shifted by shooter momentum**: `threshold = base% + MO × randint(*MO_FT_SECOND_CHANCE_ROLL)`, clamped to `[0,100]` (see `Player_Momentum_System.md § Free Throw Impact`).
5. **Bonus Thresholds**:
...
3. **Apply Secondary Check (Miss-to-Make Conversion)**
   - If `makes_shot = False`: `threshold = base_pct + MO × randint(*MO_FT_SECOND_CHANCE_ROLL)` (clamped `[0,100]`); `if randint(1,100) < threshold: makes_shot = True`
   - `base_pct` = `effective_ft_miss_to_make_second_chance(game, off_team) × 100`: home (or away crowd factor ≤1) → 40; away crowd factor 2–3 → 30; ≥4 → 20
   - Momentum shift: shooter `MO × randint(1,3)` pts (signed); MO 0 → no change. See `Player_Momentum_System.md § Free Throw Impact`
...
After the initial calculation, if the free throw is a miss, there is a chance to convert it to a make. The base probability comes from `effective_ft_miss_to_make_second_chance(game, off_team)` (`BackEnd/utils/home_crowd.py`), expressed as a percent and then shifted by the shooter's momentum (a `1–100` roll vs the threshold):
```python
if not makes_shot:
    second_chance_pct = effective_ft_miss_to_make_second_chance(game, off_team) * 100
    ft_mo = int(attrs.get("MO", 0) or 0)
    _lo, _hi = MO_FT_SECOND_CHANCE_ROLL
    threshold = max(0, min(100, second_chance_pct + ft_mo * random.randint(_lo, _hi)))
    if random.randint(1, 100) < threshold:
        makes_shot = True
        ft_made_on_second_chance = True
```

- **Base %** — **Home shooter** (`off_team.is_home_team`): `FREE_THROW_MISS_TO_MAKE_SECOND_CHANCE = 0.40`. **Away shooter**: scaled down by `home_crowd_factor` — factor ≤1 → 0.40, factor 2–3 → **0.30**, factor ≥4 → **0.20** (home-court FT pressure; see `Home_Crowd_System.md`).
- **Momentum shift** — shooter `MO × randint(*MO_FT_SECOND_CHANCE_ROLL)` percentage points (signed), clamped to `[0,100]`. Positive MO → more second-chance makes; negative → fewer; MO 0 → no change. See `Player_Momentum_System.md § Free Throw Impact`.
...
**Momentum (MO):** on a trip with **>1** FT attempted, the shooter gets a flat **−1 MO** if he **misses all** or **+1 MO** if he **makes all** (mixed → nothing), once at trip end (`mo_ft_trip_attempts`/`mo_ft_trip_makes` counters in `resolve_free_throw_logic`). FTs are not tracked on `Shot_Result_List`. See [Player_Momentum_System.md](Player_Momentum_System.md).
