...
- **End of game:** `reset_all_player_momentum(game)` zeros **every** player's MO (both teams, active + bench) at the live game-final detection (`is_final` in `api.py`), before the final save — no in-game momentum persists past the game. This live player MO system is unrelated to the deferred legacy FTD season-momentum fields. See `End_Of_Game_System.md`.
