...
- **Constants** (in `BackEnd/constants/__init__.py`): `BLOCK_RECONCILIATION_SHOOTING_FOUL_THRESHOLD` (currently **150**), `BLOCK_RECONCILIATION_BLOCK_THRESHOLD` (currently **-100**). They are **independent**: adjust one without affecting the other.
...
- **Block reconciliation:** Use **shot_score before defense penalty**; defense_block_score uses scaled height `(height_score * 10) + random(-9, 9)` plus ID/IQ. shot_score − defense_block_score: > 150 → shooting foul; < −100 → block; else → standard shot.
...
- Block-volume recalibration (July 2026): `BLOCK_RECONCILIATION_BLOCK_THRESHOLD` moved from `-150` to `-100`. A `block_funnel_tracking` diagnostic now follows eligible shots through trigger, reconciliation foul/fallback/block bands, actual blocks, and foul-owned block contacts; it is included in per-game and week-aggregate shot diagnostics.
