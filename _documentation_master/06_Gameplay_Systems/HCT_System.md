...
2. **Defense wins the encounter:** `p_event = DEF_WIN_BASE (**0.30**) × margin × agg (0.7 / 1.0 / 1.3) × …`, capped at **60%** — then a weighted roll for steal / dead-ball TO / charge; tune **`HCT_D8_DEF_WIN_BASE`**, **`HCT_D8_P_EVENT_MAX`**, **`HCT_D8_AGG_MULT`** for overall trap chaos.
...
4. **Asymmetry to know when tuning:** Same agg multiplier on both branches, but **0.30 vs 0.20 bases** and a **60% cap on defense-wins events only** mean aggressive is still **reward-heavy when the trap is winning checks** and **foul-heavy when it's losing them**; steals get **extra** agg via `steal_factor` (only knob that double-dips).
