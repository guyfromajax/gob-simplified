...
- **Free-throw second chance:** `effective_ft_miss_to_make_second_chance(game, offense_team_at_line)`. **Home** shooters always use the global default; **away** shooters use the crowd tiers above (factor ≤1 → default, 2–3 → 0.3, ≥4 → 0.2). See `Free_Throw_System.md` for how the miss→make second-chance roll is applied.
