...
5. **Score every eligible rebounder**:
   ...
   - **Offensive-rebounder discount:** offensive players' final score is multiplied by **`OREB_REBOUND_SCORE_DISCOUNT` (0.7)** — modeling the defense's box-out / positioning edge on a miss (offense and defense were otherwise scored identically). Applied in `select_rebounder_by_score`; defense scores untouched. Tune against the week-aggregate **OREB%** (D1 target ~30%; `1.0` = legacy no-discount). Note: a 0.7 *score* discount is **not** a 30% *rate* reduction — the winner is a max-of-`d6` pick, so the OREB% effect is non-linear and tuned empirically.
