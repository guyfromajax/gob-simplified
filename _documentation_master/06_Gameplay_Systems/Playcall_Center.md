...
3. **Backend application**
   ...
   - **Defense**: Applied when user’s team is on defense; persistent until user clears. API accepts full names (e.g. "2-3 Zone"); "Man Normal" is sent as `"Man"` for backend compatibility. The PCC man override applies **Base Man**; the Deny/Loose Man variants are selected by `man_defense` playbook % (not browseable here). See [`integrating_new_d_plays.md`](../projects/integrating_new_d_plays.md).
