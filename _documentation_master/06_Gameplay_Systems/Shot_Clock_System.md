...
- `BackEnd/engine/phase_resolution.py` — shot-clock-0 handling in `resolve_half_court_offense_logic`: violation vs forced shot (`violation_threshold`), Motion second-chance recalibration (`recalibration_score`, `_motion_shot_recalibrated`).
