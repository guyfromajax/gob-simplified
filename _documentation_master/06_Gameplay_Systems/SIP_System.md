## SIDE_INBOUND (SIP) System

> **⚠️ UESS-migrated — read this first.** SIP is migrated to the UESS schema: `setup_side_inbound()` now also emits `animation_steps` via **`build_sip_animation_steps`** (`BackEnd/utils/transition_bridge.py`) — a 2-step turn (step 1 = setup walk-in, all 10 players to SIP destinations + ball travels to SF; step 2 = inbound pass SF→PG, others stationary; gated on all 10 players, **no game-clock burn**). The frontend **schema playback path** (`processTurn` → `runSchemaPlaybackTurn` → `playTurn`) is **primary** and runs whenever `animation_steps[]` is present. See [`UESS_System.md`](../00_General_Systems/UESS_System.md) §2 (migration state) and §4 (`build_sip_animation_steps`).
>
> The `handleSideInbound()` / `runSideInboundSetup()` renderer mechanics documented below are the **legacy fallback** (for turns without `animation_steps`). The **backend positioning data** (`oDestinations` / `dDestinations` / `ball_spot`) described here is still emitted by `setup_side_inbound()` and **verified current** — it feeds both paths.
...
**Location:** `BackEnd/models/turn_manager.py` - `setup_side_inbound()` (~line 555); UESS emitter `build_sip_animation_steps`; legacy fallback `FrontEnd/static/js/phaser/animation/AnimationEngine.js` - `handleSideInbound()`  
**Status:** ✅ UESS-migrated (schema playback primary); legacy `runSideInboundSetup` retained as fallback  
**Scope:** Player positioning and inbound pass execution after dead ball situations

### Process Overview (legacy fallback path)

> Primary path is UESS schema playback (see banner). The flow below describes the legacy `handleSideInbound` fallback, used when a SIP turn arrives without `animation_steps[]`.
...
**Backend:**
- `BackEnd/models/turn_manager.py` - `setup_side_inbound()` method (~line 555; emits `oDestinations`/`dDestinations`/`ball_spot` **and** `animation_steps` via `build_sip_animation_steps`)
- `BackEnd/utils/transition_bridge.py` - `build_sip_animation_steps()` (UESS emitter — primary path)

**Frontend:**
- Schema playback (primary): `processTurn` → `runSchemaPlaybackTurn` → `playTurn` (see `UESS_System.md`)
- Legacy fallback: `FrontEnd/static/js/phaser/animation/AnimationEngine.js` - `handleSideInbound()`; `PassAnimationSystem.js` - `executeInboundSequence()`; `turnAnimation.js` - `runSideInboundSetup()` (line numbers drift; locate by function name)
