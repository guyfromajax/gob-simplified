...
| `BLOCK_FIGHT_RANGE_MAX` | 10 | Z random range max |
...
Used for TEAM_ATTR_CLAMPS (e.g. training, team init). Min/max clamp for generated or overridden team attributes. Full player and team clamp reference: **`../06_GMO_Supporting_Systems/Attribute_Clamp_System.md`**.

| Key | Current value | Notes |
|-----|---------------|--------|
| `shot_threshold_min` | 50 | `BackEnd/constants/shot_threshold_scale.py` |
| `shot_threshold_max` | 250 | same; span always 200, mid = min + 100 |
